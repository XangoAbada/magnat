...
Zgodnie z `K-18`. Wpis powstał przy przeglądzie zapisu sesji po M9e — nie z pracy nad M5.
Prefiks `AB-n`. Gwiazdka = zmiana zakresu albo kryterium.

| # | Zmiana | Dlaczego |
|---|---|---|
| AB-1 ★ | **Decyzja otwarta nr 16 dostaje wykonawcę: `R2-WP32` w `R2b-pieniadz-gospodarstwa.md`.** Treść punktu nie zmienia się ani o zdanie — zmienia się to, że ma adresata. Bramka 7 fazy M5 zostaje otwarta do R2, ale **z nazwanym pakietem**, a nie bez niego. Warunek „uzgodnić z M4 przed wpięciem konta" obowiązuje dalej i jest pierwszym krokiem tamtego pakietu; `K-62` opisuje zmianę kontraktu, którą to niesie | `AA-2` zapisało, że punkt zostaje otwarty, i zrobiło to poprawnie. Czego `AA-2` nie zrobiło — i czego nie zrobiła żadna z faz M6…M9 — to wskazania, **kto** go domknie. Przez cztery fazy punkt istniał jako otwarta bramka bez właściciela, a rozjazd w tym czasie urósł z +63,2 tys. zł do +163,0 tys. zł na 40 dób, bo jest mnożnikowy. To jest dokładnie ta klasa, którą wychwytuje poz. 64 wykazu R2: adres bez egzekutora działa tak samo jak brak adresu, tylko dłużej wygląda na rozwiązany |
