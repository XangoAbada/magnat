...
| Kanał ulotkowy z zakładu spoza katalogu miejsc | 0 ekspozycji | dostarcza | test `ulotka_dociera_z_zakladu_spoza_katalogu_miejsc`; **nie jest to jednak przyczyna `GF-2`** — patrz `GG-8` |
| Histogram kanałów w raporcie `m7miasto` | z żywych kampanii | **dożywotni** | `GG-8`; stary odczyt pokazywał zero na każdej granicy miesiąca |
| Ekspozycje kanału ulotkowego przez 300 dób | „0" (fałszywy odczyt) | **4 452 655** | ten sam przebieg, naprawiony przyrząd — kanał nigdy nie milczał |
...
| GG-8 ★ | **`GF-2` opisywało wadę przyrządu, nie mechanizmu — i sam przyrząd to potwierdził dopiero wtedy, gdy przestał kłamać.** Histogram kanałów w raporcie `m7miasto` liczył się z **żywych kampanii**, a kampania żyje trzydzieści dób: `close_finished` usuwa ją razem z jej pomiarem, a `ai::monthly` otwiera nowe **za** dobowym rozdaniem ekspozycji w `doba()`. Przebieg kończący się na wielokrotności trzydziestu widzi więc wyłącznie kampanie **jednotickowe** i pokazuje zero na każdym kanale. M10f mierzył `--days 300`, a 300 = 10 × 30 | Wniosek „**`Leaflet` 6 kampanii, 0 ekspozycji**" był poprawnym odczytem błędnego przyrządu. Sprawdzian, którego wtedy nie zrobiono, jest jednozdaniowy: ten sam przebieg przy `--days 70` pokazywał **18 kampanii ulotkowych i 251 940 ekspozycji**, a 70 nie jest wielokrotnością trzydziestu. Naprawa idzie w przyrząd: `Campaigns` prowadzi **licznik dożywotni per kanał**, wpięty do hasha stanu jak reszta rejestru, a raport czyta jego i mówi wprost, kiedy próbka wypadła na granicy miesiąca. Pilnuje tego test `licznik_kanalu_przezywa_wygasniecie_kampanii`. **Pomiar po naprawie, ten sam przebieg i te same trzysta dób: `Leaflet` dostarczył 4 452 655 ekspozycji** — czyli kanał nie milczał ani przez jedną dobę. Przy okazji wyszło, że **żaden inny kanał nie dostarczył ani jednej** przez trzysta dób, i to jest prawdziwa liczba, a nie artefakt: firmy nie mają za co kupić prasy ani telewizji, co jest tym samym `FF-29`, który głodzi badania i giełdę (`GF-3`). **Lekcja jest ta sama, którą M10f zapisał przy `G12`** („agregat ukrywa zero"), tylko o jedno piętro wyżej: **pomiar ze stanu chwilowego ukrywa całą historię**, a wniosek z takiego pomiaru wygląda dokładnie jak wniosek z prawdziwego |
