...
| ★ | **`Policy.domain` jest dziś ograniczona do `Pricing` i `Stock`** — walidator zwraca `DomainNotAvailable` dla `Hr`, `Production` i `Logistics` | Wykonawca akcji istnieje dla ceny i zapasu (sklep, M7c); kadr, produkcji i logistyki nikt jeszcze nie wykonuje. Polityka bez wykonawcy wyglądałaby w edytorze jak działająca i nie robiłaby nic. **Dla M9 znaczy to jedno:** przykładowa polityka „Zatrzymać ludzi" z §5.6 nie da się dziś przypiąć i jej odblokowanie należy do M7e, nie do WP8 |
