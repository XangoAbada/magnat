...
| `CD-6` | ~~`M8e-wladza-i-wybory.md`~~ → `R3-etap-7-i-dlug-strukturalny.md` | **Planowy remont linii (`EdgeState::UnderMaintenance`) należy do M8e i wariantu jeszcze nie ma.** M8b ma dwa stany krawędzi: `Ok` i `Tripped { until }`. Trzeci dopisuje się razem z tym, kto go ustawi — czyli razem z harmonogramem konserwacji operatora | `R2` zastosowane do wariantu enuma (`CC-4`). Wariant, którego nic nie ustawia, przechodzi każdy test i w raporcie wygląda tak samo jak wariant działający **Adres wygasł: M8e zamknęło się bez tej pozycji, a `plan_guard` (`R2-WP26`) złapał ją 2026-09-22. Przeniesione do `R3`.** |
...
Zgodnie z `K-18`. Poprawki do dokumentu **tej fazy** (§7) i do dokumentów podfaz
oraz faz następnych, które M8d pokazała jako nieprawdziwe albo niedopowiedziane.
Gwiazdka = zmiana zakresu albo kryterium.

| # | Dokument | Zmiana | Dlaczego |
|---|---|---|---|
| `CH-1` ★ | ten dokument, §7 (T4) | **T4 mierzy tempo, a nie stan po dziesięciu latach.** Kryterium „mediana umiejętności 18-latków różni się o ≥ 8 punktów `Q`" jest przy tempie z `data/tuning/city.ron` spełnione w **drugim** roku gry, a po dziesięciu obie strony stoją na suficie skali (100) i różnica wynosi zero. Mierzone przebiegi: szkoła 720 dób, absencja chorobowa 540 dób, straty inwentaryzacyjne bez przebiegu (funkcja czysta), domiar 90 dób pełnej gospodarki | Przypadek (3) z `K-18`: kryterium **spełnione tożsamościowo** przy dosłownym czytaniu. Ta sama korekta co `BF-3` w M7f — długość przebiegu zapisana jako kryterium, a mierzalne jest tempo |
| `CH-2` ★ | `M8e-wladza-i-wybory.md` §5.2 | **Kolejka pozwoleń już stoi i M8e jej nie buduje.** `Permit`, `PermitStatus`, `PermitOffice`, `PermitRegistry` i `permits::process_queue` (odpowiednik `sys_process_permit_queue`) są w `sim/city` od M8d, razem z przerobem urzędu, dniami wolnymi (`K-15`) i medianą czasu oczekiwania. Czego **nie ma**: `PermitPriority` (`Expedited`, `Political` — obie potrzebują gracza albo rady), `RejectReason` (odmowa wymaga przesłanki, czyli uchwały), `Applicant::Player` istnieje jako wariant bez pisarza, a `Tender` nie powstał wcale | Przypadek (2) z `K-18`: API, którego M8e używa w opisie, istnieje i nazywa się konkretnie. Zapisane, żeby M8e nie zaczął od drugiej kolejki obok istniejącej |
| `CH-3` ★ | `M8e-wladza-i-wybory.md` §5.8 | **System `city.Tax` wykonuje siedem kroków, nie pięć.** Po `CB-2` tabela systemów miała pokazywać jeden system zamiast czterech; po M8d ten sam system robi dodatkowo dobę urzędów (`law::step_day`, kolejka pozwoleń) i — raz w miesiącu — przeliczenie jakości placówek, publikację pokrycia, ubytki inwentaryzacyjne i decyzję zakładów o szarej strefie. Kadencja i wyłączność bez zmian; kolejność wobec `economy.Market` bez zmian | Przypadek (4) z `K-18`: tabela systemów jest jedynym miejscem, w którym kolejność kroków fazy widać naraz, a po M8d przestała opisywać to, co się dzieje |
| `CH-4` ★ | `M8e-wladza-i-wybory.md` | **Miasto finansuje placówki „na papierze": plan wydatków wychodzi na konto reszty świata, a nie do nich.** `budget::close_month` przelewa udział kierunku wydatku na `rest_of_world` (świadomy sufit z M8a, `ponytail:` w `budget.rs`), a jakość placówki liczy się z **planu**, nie z przelewu. Nauczyciel dostaje pensję z listy płac, której `PayrollOutbox::take()` nie ma konsumenta od M7b. Zamknięcie tej pętli — miasto jako pracodawca — jest pakietem M8e i wymaga `FirmKey` dla miasta (`Owner::City` czeka w `sim/firms` od M7a) | Przypadek (5) z `K-18`: łańcuch „budżet → pensja nauczyciela → jakość szkoły" ma dziś dwa ogniwa z trzech i żaden pakiet nie jest właścicielem trzeciego. Skutek jest dziś prawdziwy (jakość zależy od planu i od obsady), ale pieniądz się w nim nie rusza |
| `CH-5` | ~~`M8e-wladza-i-wybory.md`~~ → `R3-etap-7-i-dlug-strukturalny.md` | **`Policy::MinWage` ma gotowego konsumenta i jest nim inspekcja pracy.** Urząd otwiera dziś sprawę, gdy ponad połowa umów zakładu stoi poniżej `wage_floor_bp` dolnych widełek roli z `data/jobs/roles.ron`. Uchwała o płacy minimalnej podmienia **próg**, nie mechanizm: sprawa, dowody, kara i wpis do budżetu są już na miejscu | `D8` fazy mówi „uchwalana przez M8, egzekwowana przez M7 przy ustalaniu ofert pracy". Po M8d egzekucja ma drugą stronę — karną — i ona jest tutaj **Adres wygasł: M8e zamknęło się bez tej pozycji, a `plan_guard` (`R2-WP26`) złapał ją 2026-09-22. Przeniesione do `R3`.** |
| `CH-6` ★ | ~~`M8e-wladza-i-wybory.md`~~ → `R3-etap-7-i-dlug-strukturalny.md` | **Budżet urzędu kontrolnego nie ma pisarza.** `Agency.budget` jest polem, które nikt nie wypełnia: urzędy finansuje `SpendCategory::Administration` bez rozbicia per urząd, a realnym ograniczeniem jest liczba inspektorów, brana z obsady urzędów miejskich (`ServiceKind::Office`). Rozbicie budżetu na pięć urzędów jest decyzją burmistrza, czyli M8e | `R2` zastosowane do pola struktury. Zapisane, bo z istnienia pola łatwo wnieść, że coś je wypełnia **Adres wygasł: M8e zamknęło się bez tej pozycji, a `plan_guard` (`R2-WP26`) złapał ją 2026-09-22. Przeniesione do `R3`.** |
| `CH-7` | `M9c-gracz-inspekcja-nakladki.md` | **Karta usługi publicznej i karta sprawy mają komplet danych.** Cztery zakładki z decyzji właściciela produktu (Obsada · Zasięg · Finansowanie · Jakość) czyta się z `PublicServices::by_site(SiteId)`; sprawy zakładu z `Enforcement::of_site(SiteId)`, a pojedynczą z `Enforcement::get(CaseId)`. `Case` niesie `subject: SiteId` i `firm: FirmId`, więc odnośnik idzie w obie strony bez parsowania tekstu. Powód `DecisionReason::ServiceQuality` niesie **cztery liczby obok wyniku**, więc zakładka „Jakość" nie musi liczyć niczego sama | Przypadek (2) z `K-18`, w tej samej postaci co `CF-7` po M8c: API, którego M9c używa w opisie, istnieje i nazywa się konkretnie |
| `CH-8` ★ | `M10d-gielda-przejecia-ubezpieczenia.md` | **Przymusowy podział czeka na rynek kontroli nad firmą.** `Remedy::ForcedDivestiture` wykonuje się dziś **zamknięciem** zakładu, bo nie ma komu go sprzedać. Udział rynkowy spada tak samo, ale gospodarka traci zdolność produkcyjną zamiast przenieść ją do innego właściciela. Wejście dla M10 jest jedno i nazwane: `naloz_srodek` w `sim/city::law`, ramię `AgencyKind::Antitrust` | `ponytail:` sufit nazwany w kodzie, z adresem. Zapisane tutaj, bo M10d jest dokumentem, który go zdejmie |
| `CH-9` | `R2-naprawy-po-M11.md` | **Dwie pozycje do wykazu, obie znalezione przy budowie kanałów skutków.** (1) `Parcel.land_value_per_m2` **nie zmienia się w trakcie gry** — jedyne dwa zapisy w repozytorium to `value::pass_1` i `pass_2`, wołane raz przy generacji miasta. Każdy kanał skutku kończący się na wartości gruntu (parki, odpady, hałas) jest przez to niewykonalny, a nie tylko odłożony. (2) `ServiceKind::Waste` nie ma **ani jednego archetypu** w `data/buildings/public.ron`: `SpendCategory::Waste` ma udział w planie wydatków od M8a, ale pieniądze idą na usługę, której w mieście nie ma | `R2` w obu przypadkach: mechanizm, który przechodzi każdy test i w raporcie wygląda tak samo jak działający. Pierwsza pozycja blokuje trzy kanały skutków naraz, więc jest warta osobnego wiersza |
| `CH-10` | `M12b-zapis-i-replay.md` | **Zasób `City` niesie od M8d cztery nowe składniki stanu**: placówki (obsada, jakość, stan budynku, obłożenie), rejestr pozwoleń z kolejkami urzędów, urzędy kontrolne ze sprawami i tablicę pokrycia usług. Wszystkie wchodzą do hasza przez `HashState for City`; **`ServiceCoverage` wchodzi dwa razy nie będzie** — kopia w świecie jest migawką, a haszuje się oryginał w `PublicServices` | `CE-15` po M8c mówi, że pełny zapis zasobów świata jest zakresem M12b. Lista rośnie i warto, żeby rosła w jednym miejscu |
| `CH-11` ★ | `M8e-wladza-i-wybory.md`, `M10-glebia.md` | **Przerwa między zdarzeniami (`cooldown_days`) jest przerwą definicji, a nie podmiotu.** `Events::in_cooldown` patrzy na `last_end_day` całej definicji, więc dla zdarzenia o zakresie `Firm` albo `Site` jest **globalnym licznikiem udającym lokalny**: jedna firma dostaje kontrolę, całe miasto ma spokój. Zdarzenia polityczne M8e (uchwała, protest wywołany decyzją rady) i rynkowe M10 mają ten sam zakres i ten sam mechanizm, więc ta sama pułapka czeka tam nietknięta. Reguła: **przerwa per definicja dla zdarzenia miejskiego (susza, embargo), `max_concurrent` + `Events::busy(def, scope)` dla zdarzenia per podmiot** | Pomiar w M8d (`CG-20`): 3 kontrole na 200 dób gry przy hazardzie, który zapowiadał sześćdziesiąt. Błąd nie objawia się jako awaria ani jako martwa definicja — zdarzenie zachodzi, tylko dwadzieścia razy rzadziej, niż mówi krzywa, i żaden test tego nie łapie |

## Zmiany wpisane po M8e

Zgodnie z `K-18`. Poprawki do dokumentów **faz następnych** i do wykazu `R2`,
które M8e pokazała jako nieprawdziwe albo niedopowiedziane.
Gwiazdka = zmiana zakresu albo kryterium.

| # | Dokument | Zmiana | Dlaczego |
|---|---|---|---|
| `CJ-1` ★ | `M9c-gracz-inspekcja-nakladki.md` | **Karta rady ma komplet danych i jest singletonem, tak jak zapowiadał `K-62`.** Sześć zakładek czyta się z pięciu pól zasobu `City`: Skład — `City.gov` (`mayor`, `mayor_pref.dominant()`, `council`, `term_start`, `approval_bps_by_district`); Polityka i podatki — `City.policies.in_force(t)` plus `TaxCode::rate_of` na siedmiu daninach; Budżet — `City.budget` bez zmian od M8a; Wybory — `City.election` (`candidates`, `result.per_district`, `vote_log`); Przetargi — `City.tenders.all()` i `contracts_in_force(t)`; Dlaczego — `City.reasons` (pierścień 256) plus `Government.signals`, czyli **siedem zmierzonych liczb obok decyzji**. `Candidate.citizen` jest indeksem encji mieszkańca, `Bid.bidder` to `FirmKey`, `Case.subject` to `SiteId` — graf odnośników jest kompletny w danych i nic nie wymaga parsowania tekstu | Przypadek (2) z `K-18`, w tej samej postaci co `CF-7` po M8c i `CH-7` po M8d: API, którego M9c używa w opisie, istnieje i nazywa się konkretnie |
| `CJ-2` ★ | `M9c-gracz-inspekcja-nakladki.md`, `M9a-szkielet-gry-i-komendy.md` | **Wejście gracza do wyborów jest jedno i nazywa się `Election::back_candidate`.** Metoda przyjmuje `Backer::Player`, kwotę i `Legality`, zwraca gotowy `DecisionReason::CampaignBacked` i **nie przelewa pieniędzy** — przelew robi wołający przez `Books::transfer` z `TxKind::CampaignDonation` (`K-66`, `CI-6`). Drugim wołającym jest już dziś firma AI, więc gracz wchodzi w mechanizm działający, a nie w zaślepkę. Wsparcie nielegalne otwiera sprawę w urzędzie antymonopolowym tą samą drogą co każda inna sprawa (`Enforcement::otworz`, publiczne od M8e) | Wykonanie `K-11` dla wyborów: gracz i AI mają ten sam kod, a różnica leży w tym, ile kto daje i komu |
| `CJ-3` | `M9c-gracz-inspekcja-nakladki.md` | **`Subject` dostaje w M8e trzy nowe podmioty do obsłużenia i wszystkie mają już `id`:** rada (singleton, bez ładunku), przetarg (`TenderId(u32)`) i pozwolenie (`PermitId(u32)`). `Election` **nie dostaje wariantu** — jest jedna naraz i jest zakładką karty rady, dokładnie jak w decyzji właściciela produktu z 2026-09-17. Kandydat linkuje do mieszkańca przez `Candidate.citizen`, a fundator przez `Backer::Firm(FirmKey)` — obie strony są identyfikatorami, nie napisami | Wykonanie wymagania z tabeli „Zmiany wpisane po decyzji właściciela produktu" w dokumencie `M8e` |
| `CJ-4` ★ | `M9e-panele-czas-kariera.md` | **`Policy::ParkingFee` czeka na punkt podmiany po stronie M4, a nie na M8e** (`CI-2`). Cennik parkingu ma czytelnika (porównanie środków transportu, `oracle/offers.rs`) i jest dziś wszędzie zerem, ale jedyna droga zapisu to `TrafficOracle::set_parking(&mut self, …)` wołane przy budowie świata, a oracle żyje za `Arc` w `AgentSources`. Faza, która chce, żeby opłata parkingowa działała, dokłada **metodę z `&self`** po stronie `sim/traffic` — tak samo jak `set_weather` (`K-26`) | Przypadek (1) z `K-18`. Zapisane, bo wariant wygląda na oczywisty i przy pierwszym podejściu kosztował pół dnia |
| `CJ-5` ★ | `M10d-gielda-przejecia-ubezpieczenia.md`, `M10b-marka-i-media.md` | **Operator sieci nadal nie ma rachunku wyników i to blokuje dwie rzeczy naraz** (korekta `CD-5` i `CF-5`). `Policy::TariffCap` **działa**: uchwała przycina `UtilityNetwork.tariff.per_unit` i licznik zakładu liczy po nowemu od następnego kroku. Czego dalej nie ma: strony kosztowej operatora, więc pętla „sufit taryfy → operator tnie konserwację → rośnie hazard awarii bloku" z §5.4 ma **trzy ogniwa z czterech** — sonda `SourceMaintenanceOverdueDays` jest gotowa od M8c i jest czynnikiem hazardu, brakuje wyłącznie tego, kto konserwację tnie. Razem z tym `EdgeState::UnderMaintenance` (`CD-6`) zostaje wariantem bez pisarza, bo planowy remont jest decyzją operatora. Obie rzeczy zdejmuje ta sama zmiana: operator jako firma z księgą (decyzja `D10`) | `R2` zastosowane do wariantu enuma i do pętli naraz. Zapisane w jednym wierszu, bo to jest jedna zmiana, a nie dwie |
| `CJ-6` | `M10b-marka-i-media.md` | **`MediaExposure` wyborcy ma dziś jedną postać i jedno wejście** (wykonanie `D7`): `Candidate::reach_bp(voters, tuning)` liczy złotówki kampanii na jednego wyborcę dzielnicy i mnoży je przez `media_reach_bp_per_zl` z `data/city/government.ron`; pieniądz spoza rejestru wpłat waży `illegal_reach_mul_bp` razy więcej. M10 podmienia **ciało tej jednej metody** na zasięg realnych tytułów, bez zmiany sygnatury i bez dotykania funkcji użyteczności wyborcy. Odbiorcą wpłaty jest dziś `RestOfWorld` (`K-66`) — to jest miejsce, w które M10 wstawi konto wydawcy | Przypadek (2) z `K-18`: API istnieje i nazywa się konkretnie. `D7` obiecywał podmianę bez zmiany sygnatury i po M8e da się wskazać, której |
| `CJ-7` ★ | `M10e-relacje-i-zwiazki.md` | **Więzi osobiste w głosowaniu to dziś „kandydat z mojej dzielnicy" i graf relacji M3 nie jest w to wpięty.** §5.7 wymienia więzi jako jeden ze składników preferencji wyborcy; wykonane jest przybliżenie, które nie kłamie o swojej naturze — `VoteDriver::Ties` dostaje stałą premię, gdy kandydat mieszka w dzielnicy wyborcy. Wpięcie `RelationSlab` wymaga przejścia po relacjach każdego wyborcy przy każdych wyborach, czyli pracy rzędu populacja × stopień grafu raz na kadencję, i **ma drugiego konsumenta dopiero w M10** (związki zawodowe, rody). Do tego czasu przybliżenie zostaje, bo mierzy właściwy kierunek | Przypadek (3) z `K-18`: składnik zapisany w §5.7 jest wykonalny, ale jego pełna postać potrzebuje konsumenta, którego M8e nie ma |
| `CJ-8` | `M12b-zapis-i-replay.md` | **Zasób `City` niesie od M8e pięć nowych składników stanu**: władzę (`Government` — burmistrz, rada, poparcie per dzielnica, rejestr histerezy, licznik uchwał), zbiór uchwał z pełną historią, trwające wybory z wynikiem per obwód, rejestr przetargów z umowami oraz **stawki siedmiu danin**. Wszystkie wchodzą do hasza przez `HashState for City`; **pierścień powodów `City.reasons` nie wchodzi** — to jest wyjaśnienie decyzji, a nie sama decyzja, i nadpisuje się niezależnie od niej, tak samo jak `Market::budget_log` i `Events::reasons`. Stawki wchodzą do hasza **dopiero od M8e** i to jest zmiana wobec M8a: do tej pory kodeks był daną z `data/`, a od uchwały rady jest stanem | Lista rośnie od `CE-15` i warto, żeby rosła w jednym miejscu (`CH-10`) |
| `CJ-9` | `R2-naprawy-po-M11.md` | **Dwie pozycje do wykazu.** (1) `PayrollOutbox::take()` nadal **nie ma konsumenta** — czeka po raz szósty (M7b, M7f, M8a, M8d, M8e) i blokuje łańcuch „budżet → pensja nauczyciela → jakość szkoły" z `CH-4`, bo miasto nie może być pracodawcą, dopóki wypłaty nie mają odbiorcy. Wynagrodzenia płyną dziś z `rest_of_world` do gospodarstwa (`pay_incomes`), a lista płac firm rośnie w skrzynce, której nikt nie opróżnia. (2) Urząd antymonopolowy prowadzi od M8e sprawy z **dwóch** przesłanek: udziału rynkowego i wpłaty poza rejestrem wpłat kampanijnych — a druga jest przestępstwem, nie praktyką ograniczającą konkurencję. Rozdzielenie wymaga szóstego urzędu (`AgencyKind::Prosecution`), a kolejność wariantów jest kontraktem indeksu, więc wariant dopisuje się **na końcu** | `R2` w obu przypadkach: mechanizm, który przechodzi każdy test i w raporcie wygląda tak samo jak działający. Pierwsza pozycja jest przenoszona po raz szósty i to jest dokładnie ten wzorzec, który `R2-WP26` ma złapać |
| `CJ-10` | `M10-glebia.md`, `M9d-jezyk-regul.md` | **`EventCause::{Policy, Player}` nadal nie mają pisarza — i to jest rozstrzygnięcie, nie zaległość** (korekta `CF-4`). Uchwały M8e działają **na stanie**: stawka podmienia kodeks, sufit taryfy przycina pole sieci, limit emisji przestawia próg urzędu. Zdarzenie nie nadaje się do tego z powodu, który jest jego zaletą w każdym innym zastosowaniu: `ParamOverlay` **wraca do wartości zastanej** po wygaśnięciu (`CE-4`), a uchwała wracać nie może — po to ma `sunset`, żeby wygasać jawnie i na własnych warunkach. Wariant `Policy` w `EventCause` przyda się dopiero temu, kto zechce, żeby uchwała **wywołała** zdarzenie (rada ogłasza dzień bez samochodu), a nie żeby nim była | Przypadek (1) z `K-18`: stan, na którym `CF-4` stał, wygląda po M8e inaczej — i to na korzyść |
...
Zgodnie z `K-18`. Wpis powstał przy przeglądzie zapisu sesji po M9e — nie z pracy nad M8.
Prefiks `CK-n`. Gwiazdka = zmiana zakresu albo kryterium.

| # | Dokument | Zmiana | Dlaczego |
|---|---|---|---|
| `CK-1` ★ | `R2-naprawy-po-M11.md` | **Obie pozycje obiecane przez `CJ-9` istnieją teraz w wykazie jako 58 (`PayrollOutbox` bez konsumenta, pakiet `R2-WP30`) i 59 (rozdzielenie urzędu antymonopolowego, pakiet `R2-WP31`, kontrakt `K-73`).** Między obietnicą a wpisem minęły dwie podfazy M9 i wpis powstał dopiero z przeglądu zapisu sesji, a nie z pracy nad żadną fazą | `CJ-9` zrobił dokładnie to, czego wymaga `K-18`: nazwał dokument docelowy i opisał treść. Dokument docelowy nigdy jej nie dostał — i **nic tego nie sprawdzało**, bo tabele korekt nie mają egzekutora, w przeciwieństwie do rejestru długu. To jest pozycja 64 wykazu R2 i drugie pół pakietu `R2-WP26`. Warto zapisać ironię: `CJ-9` kończy się zdaniem „to jest dokładnie ten wzorzec, który `R2-WP26` ma złapać" — i sam mu uległ |
| `CK-2` | `R2-naprawy-po-M11.md` | **Zmiana nazwy `UtilityKind` → `UtilityService` z `CB-1` jest wykonana w połowie**: oba identyfikatory żyją w `engine/core` (`vocab.rs`, `decision.rs`, `lib.rs`) i oba mają czytelników. Pozycja 63 wykazu, pakiet `R2-WP22` | Dwie nazwy na jedno pojęcie kosztują przy każdym `match` i przy każdym czytaniu. Zarządzenie zmiany nazwy bez testu, który pilnuje, że stara zniknęła, kończy się tym stanem za każdym razem |
