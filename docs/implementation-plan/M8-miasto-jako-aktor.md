...
| `CH-5` | `M8e-wladza-i-wybory.md` | **`Policy::MinWage` ma gotowego konsumenta i jest nim inspekcja pracy.** Urząd otwiera dziś sprawę, gdy ponad połowa umów zakładu stoi poniżej `wage_floor_bp` dolnych widełek roli z `data/jobs/roles.ron`. Uchwała o płacy minimalnej podmienia **próg**, nie mechanizm: sprawa, dowody, kara i wpis do budżetu są już na miejscu | `D8` fazy mówi „uchwalana przez M8, egzekwowana przez M7 przy ustalaniu ofert pracy". Po M8d egzekucja ma drugą stronę — karną — i ona jest tutaj |
| `CH-6` ★ | `M8e-wladza-i-wybory.md` | **Budżet urzędu kontrolnego nie ma pisarza.** `Agency.budget` jest polem, które nikt nie wypełnia: urzędy finansuje `SpendCategory::Administration` bez rozbicia per urząd, a realnym ograniczeniem jest liczba inspektorów, brana z obsady urzędów miejskich (`ServiceKind::Office`). Rozbicie budżetu na pięć urzędów jest decyzją burmistrza, czyli M8e | `R2` zastosowane do pola struktury. Zapisane, bo z istnienia pola łatwo wnieść, że coś je wypełnia |
