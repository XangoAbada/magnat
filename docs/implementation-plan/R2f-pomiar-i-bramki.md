...
| F-17 ★ | **Lista crate'ów w jobie `bench-guard` nie obejmowała trzech z szesnastu, które mają benchmarki — i widać to było dopiero, gdy brak wpisu stał się błędem.** `magnat-agents` miał **sześć pozycji w linii bazowej i ani jednej w przebiegu**, `magnat-render` jedną w tej samej sytuacji, a `magnat-supply` **ani jednej po żadnej ze stron**: benchmarki są w repozytorium od M6 i nikt ich nigdy nie zmierzył. Wszystkie trzy dopisane; lista pokrywa od R2f każdy crate z sekcją `[[bench]]` | `D-R8` mówił o trzynastu pozycjach bez wpisu w linii bazowej i to nie była cała choroba. Brak wpisu przy zmierzonym benchmarku i brak pomiaru przy istniejącym wpisie to **ta sama usterka widziana z dwóch stron**, a obie były ciche: pierwsza wypisywała `NOWY` z kodem zero, druga `BRAK` z kodem zero. Przy okazji upadło założenie, że `instancing_bench` potrzebuje karty graficznej — nie dotyka jej ani razu, mierzy kompakcję instancji po stronie procesora |
