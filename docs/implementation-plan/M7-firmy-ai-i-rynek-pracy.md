...
## Zmiany wpisane po M7f

Poprawki dokumentu **fazy** naniesione w trakcie podfazy M7f (`K-18`). Korekty samej
podfazy są w tabeli „Zmiany wpisane po M7f" w `M7f-makro-i-domkniecie.md`.
Gwiazdka = zmiana zakresu albo kryterium.

| # | Co | Dlaczego |
|---|---|---|
| `BF-1`★ | **§6 „Dostarczam" zmienia adres tieru strategicznego.** `rank_variants` i `RankedVariants` nie są typami `sim/firms`, tylko `sim/macro`; tier strategiczny ma własny system `macro.WhatIf` w `sim/macro`, a nie ramię w `sim/economy::ai_run`. `sim/firms` dostaje w zamian `StrAction`, `Outlook`, `StrategicOutlooks` i `decide_strategic(&FirmView)`. Podpis wykonawcy ma dziś postać `Market::run_firm_ai(&mut Firms, &AiInputs, &[(Tier, Vec<FirmKey>)], Tick)` | Rozstrzygnięcie w `K-54`: `sim/macro` woła jądro (`K-50`), więc stoi **nad** `sim/economy`, a `rank_variants` w `sim/firms` zamknąłby cykl `firms → macro → economy → firms`. Wychodzi z tego gwarancja mocniejsza, niż §7.7 pkt 4 obiecywał: `Outlook` nie ma pola pieniężnego i `sim/firms` nie ma czym go dodać, więc próg absolutny na prognozie jest **niewyrażalny**, a nie zakazany |
| `BF-2`★ | **§7.6 dostaje pomiar doby pełnego miasta i jest to inna liczba, niż zapowiadał wiersz „doba gry, 10 tys. firm, wszystkie systemy M7 ≤ 300 ms".** Zmierzone na mieście 4 km: `m7miasto` **2,68 s/dobę**, `m5shop` (ten sam świat bez warstwy firm) **2,44 s/dobę** — czyli **wszystkie systemy M7 razem kosztują ~0,24 s/dobę**, a reszta to doba mieszkańca, ruch, detal i łańcuch dostaw | Cel z §7.6 mierzył niewłaściwą rzecz: „doba gry" to nie jest doba fazy M7, tylko doba całej symulacji, i 300 ms nie starczy na nią od M4. Właściwa liczba to **przyrost** i ten przyrost mieści się w budżecie z zapasem. Przy okazji: `ai::operational` **51,6 ns** i `ai::tactical` **21,3 ns** z `BD-3` nie drgnęły, a tier strategiczny woła model makro ~0,15 raza na dobę (22 uporządkowania w 150 dobach) — bo firma pyta tylko wtedy, gdy ma o co pytać |
| `BF-3`★ | **§7.7 pkt 1 i 2 są niemierzalne w zapowiedzianej postaci i mierzą się inaczej.** 200 zestawów × 5 wariantów × 90 dób **faktycznego przebiegu mezo** to ~65 godzin na jedno uruchomienie testu przy zmierzonych 2,68 s/dobę. Testuje się: kierunek znany z konstrukcji, `decisive_winner() == None` przy wariantach identycznych (pkt 3, 100 %) i **zależność marginesu od tego, od czego zależy wariancja** | Przypadek (3) z `K-18`: kryterium niemierzalne. Margines **stały** przechodziłby każdy test porównawczy i nie mówiłby nic — dlatego test sprawdza jego zmienność (rośnie z horyzontem, maleje z liczbą klientów), a nie wartość. Pełne uporządkowanie wobec mezo należy do M10, który ma `dry_run()` i przebieg „na sucho" o dwa rzędy szybszy. Szczegóły: `BE-6` |
| `BF-4`★ | **Artefakt z §1 nazywa się `m7miasto`, liczy w dobach i stawia 215 firm, nie 6–10 tys.** To jest odpowiedź na `AS-6`, odłożone w M7a: liczba firm wynika z Etapu 7 generatora (jedna firma na ~130 mieszkańców), a nie z warstwy M7. Powstawanie firm dokłada do niej kilka procent rocznie i **celowo nie więcej** (`BE-8`) | Obietnica „6–10 tys. firm przy 150 tys. mieszkańców" znaczy jedną firmę na 15–25 osób i jest założeniem o **gęstości przedsiębiorczości miasta**, czyli parametrem generatora (M2 §5.8, Etap 7), a nie wynikiem rynku pracy ani AI firm. M7 nie ma czym jej spełnić inaczej niż zalewając miasto firmami bez popytu — a to wywraca pasmo z §7.10 i bramkę G10. Adres: **M2** przy najbliższej korekcie gęstości zabudowy usługowej; do tego czasu §1 opisuje proporcję, a nie liczbę bezwzględną |
| `BF-5` | **Bramki balansatora rosną o dwie i balansator mierzy od teraz pełne miasto.** G10 (populacja firm w ±40 % wartości startowej) i G11 (mediana bezrobocia po rozbiegu w ⟨3 %, 12 %⟩). `run::single` buduje świat przez `full::setup`, czyli z rejestrem firm, rynkiem pracy, upadłością i makrem | „Bramki G1–G9 opisywały gospodarkę, w której nikt nie zatrudniał, nie płacił i nie bankrutował" — po zszyciu opisują to miasto, które widzi gracz. Zmierzone po przestawieniu: **G1–G10 zielone, G8 (determinizm) też**, czyli dołożenie czterech systemów wyłącznych i modelu makro nie ruszyło ani kalibracji cen, ani powtarzalności. Pełny wynik w dzienniku `00-postep.md` |
| `BF-6`★ | **G11 mierzy medianę z ostatnich trzydziestu dób, a nie ze wszystkiego po rozbiegu — i to jest wniosek z pomiaru, nie ostrożność.** Bramka liczy się tylko w przebiegu, w którym wakatów jest nie więcej niż ludzi w sile roboczej; inaczej zgłasza się jako doradcza z podaniem stosunku | Pierwszy pomiar dał mediany **2,0–3,3 %** przy progu 3 % i wyglądał na zepsutą gospodarkę. Przyczyna jest inna: **siła robocza w przebiegu rośnie** — miasto startuje z obsadą z generacji, a ludzi dokłada migracja (zmierzone: 1 929 osób w dobie 1, 12 291 w dobie 120) — więc przez pierwsze miesiące prawie każdy, kto już wszedł na rynek, pracuje. Mediana od 60. doby mówi, **jak szybko miasto się zaludnia**; mediana ostatnich trzydziestu dób z tych samych czterech przebiegów daje **6,4–10,9 %** i mówi o gospodarce. To jest ten sam wybór, którym G4 wyznacza „nowy poziom" ceny po szoku. Guard na mianowniku zostaje niezależnie: miasto, w którym etatów jest więcej niż ludzi, nie może mieć trzyprocentowego bezrobocia, a jego pomiar mówiłby o gęstości etatów z generatora (`AT-1`), nie o rynku pracy |
| `BF-7` | **`PayrollOutbox` czeka po raz czwarty i dostaje adres M8 razem z blokadą nazwaną w `BD-6`.** Brakuje `seller_site` w `supply::Settlement`: rozliczenie B2B niesie sprzedawcę jako `FirmId`, więc `SitePnlMonth.revenue` ma **tylko zakład handlowy**, a wypłata realnej listy płac z konta zakładu produkcyjnego wywróciłaby saldo każdej firmy przemysłowej w pierwszym miesiącu | M7f nie ruszył `sim/supply`, bo przypisanie utargu do zakładu zmienia rozliczenie każdej dostawy B2B i jest zmianą fazy M6, nie domknięciem fazy M7. `BD-6` przewidywał ten adres wprost („jeśli M7f go nie ruszy — adres M8"), a M8 potrzebuje tego samego pola do podatku od zakładu, więc zrobi je raz zamiast dwa razy |
| `BF-8` | **Dziedzina `Hr` w walidatorze polityk zostaje zablokowana, a metryka `MachineUtilization` nadal zwraca „nie wiem" — oba z adresem M9.** `BD-7` adresował je do M7f | Wykonawca `RaiseWage` potrzebuje `&mut Firms` **wewnątrz** runnera polityk półkowych, a ten stoi nad `Shop`; wykonawca `Hire` potrzebuje dodatkowo tworzenia stanowisk. Odblokowanie dziedziny z **jedną** działającą akcją z dwóch byłoby dokładnie tym, przed czym `DomainNotAvailable` broni: gracz napisałby regułę, w której połowa nic nie robi. Właściwym miejscem jest **walidacja per akcja**, a ta należy do języka reguł, czyli do M9 (`K-11`) |
| `BF-10`★ | **Pięcioletni przebieg z §7.10 wykonany i pokazuje, że rynek pracy w tym horyzoncie „wysycha": bezrobocie schodzi do 0,2 %.** 1800 dób, ziarno 1, 3 tys. mieszkańców startowych: firm 215 → 242 (+12,5 %, pasmo §7.10 dotrzymane), 26 firm założonych przez mieszkańców, 1 wejście sieci, 715 postępowań upadłościowych, 28 290 akcji AI i **1 063 331 zapisanych powodów** (wyjaśnialność 100 %), 1,08 s/dobę. Pieniądz: `check_conservation` zielone, kreacja kredytowa +33,2 mln zł, kapitał zewnętrzny +0,54 mln zł, reszta dryfu −0,06 % sumy świata i tego samego rzędu co w `m5shop` (rejestry ruchu M4 bez kont) | Przyczyna jest ta sama, co przy `BF-4` i `BF-6`, i to jest jej trzecie niezależne potwierdzenie: **miasto ma więcej etatów niż ludzi**. Po pięciu latach siła robocza urosła migracją do 19 328 osób, a 12 032 etaty nadal stoją puste — więc każdy chętny ma gdzie pójść i stopa schodzi do tarcia. Bramka scenariusza (`--expect-unemployment 30:120`) zapala się na czerwono **poprawnie** i ma się zapalać: to jest sygnał o gęstości etatów z Etapu 7 generatora (`AT-1`), a nie o rynku pracy M7b, który przez 120 dób daje 6,4–10,9 %. Adres: **M2**, ten sam co `BF-4`. **Drugie spostrzeżenie warte zapisania:** 715 postępowań przy 242 firmach znaczy, że firma upada wielokrotnie — `CorpFinance` nie zamyka jej po podziale masy, tylko zostawia w rejestrze. Dla §7.10 („≥ 1 bankructwo na 100 firm rocznie") to jest **powyżej** pasma, a nie poniżej, więc kryterium jest spełnione od złej strony i wymaga pomiaru **firm**, nie **spraw**. Adres: **M8**, razem z wierzycielem podatkowym |
| `BF-9` | **Decyzje otwarte zamknięte w M7f: `D8` i `D10`.** `D8` (progi klas S1/S2/S3) — rozstrzygnięte **przez niebudowanie klas**: budżet tieru strategicznego wyraża się w liczbie firm na dobę (`FirmKey % 90` rozkłada je po kwartale, `R4`), a firma pyta model **tylko wtedy, gdy ma o co pytać** — `propose_variants` z jednym wariantem nie idzie do rankingu wcale. Zmierzone: 22 uporządkowania na 150 dób przy 215 firmach, czyli ~0,15 wywołania modelu na dobę. Ranking po aktywach nie kupiłby nic ponad to. `D10` (punkt emisji kapitału sieci) — kanał istnieje po stronie M5 od M5d i nazywa się `Books::inject_external_capital` z pozycją `external_capital_in`; osobny wariant `MoneyEmission::ExternalCapital` nie powstał i nie jest potrzebny, bo rejestrem podaży jest `MoneySupplyLedger`, a nie enum. **Zostają otwarte z adresem M9:** `D9` (jawność ofert pracy gracza), `D11` (limit przeciągania menedżerów), `D14` (widoczność „co jeśli" dla gracza) — wszystkie trzy wymagają gracza, a nie kodu | Ta sama droga, którą M7c zamknął `D18`, a M7e `D8` w części: rozstrzygnięcie przez niebudowanie jest rozstrzygnięciem, jeśli się je zapisze |
