...
| **D6** | **Wykonana w M10e zgodnie z propozycją: strajk zatrzymuje zakład, nie firmę.** Zakres zdarzenia `social/strike` zmienił się z `Firm` na `Site` (`K-89`), a związek formuje się na spójnej składowej grafu relacji **wśród pracowników jednego zakładu**. `UnionScope::{Firm, Branch}` z planu §5.9 **nie powstaje**: eskalacja na całą firmę nie ma dziś ścieżki, którą mogłaby zajść, a wariant bez wyzwalacza wygląda w kodzie tak samo jak działający (`K-67`). |
| **D10** | **Wykonana w M10e, ale z innego źródła, niż zapowiadał kontrakt — i to jest korekta M10 §6, nie wykonanie.** `TravelTimeMatrix` w świecie świeżo postawionym jest **pusta**, bo wypełniają ją obserwacje przejazdów mezo, a historia „na sucho" żadnego nie wykonuje; jej zdjęcie dałoby tę samą płaską stałą, którą miało zastąpić. `CommuteMatrix` wypełnia się więc z **geometrii miasta** (środki ciężkości dzielnic liczone z domów mieszkańców, a dla dzielnicy przemysłowej z jej zakładów, przez nominalną prędkość miejską 400 m/min), a rekrutacja w makro przestaje być zamknięta w granicach dzielnicy: pula jest **ważona gotowością do dojazdu**, liniowo od pełnej wewnątrz dzielnicy do zera przy godzinie w jedną stronę. Kontrakt brzmi od tej chwili „M10 wypełnia z geometrii, M4 uściśla obserwacjami, kiedy są". **Czego to nie domyka:** bramki 1 i 3 Etapu 10 nie zostały ponownie zmierzone przebiegiem — pomiar należy do M10f razem z resztą przebiegu balansatora fazy (`FF-24`). |
