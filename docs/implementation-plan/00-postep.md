...
- [~] **M3d** Populacja i UI — `M3d-populacja-i-ui.md` (WP10 ✅, WP11 🔶, WP12 🔶, WP13 ✅).
  Etap 8 i warstwa systemów ECS zamknięte; z WP11 i WP12 brakuje **graficznej połowy**:
  bufora ID w renderze, klikania w pieszego, rysowania pieszych w widoku 3D i panelu
  `egui` (korekta H-15). Wszystko, co da się sprawdzić bez GPU — model karty, oś dnia
  z planem i realizacją, renderer powodów, lokalizacja pl/en, sterowanie czasem
  i selekcja — jest i ma testy

Bramki fazy:

- [x] **1. Pakiety robocze** — WP1–WP10 i WP13 wg własnych kryteriów; WP11 i WP12
  częściowo (patrz wyżej). Bramka zamknie się razem z H-15
...
- [x] **4. Budżety** — Etap 8 metropolii 26,9 s (próg 30 s), miasta 8 km 3,1 s (próg 8 s
  ...
  120 tys.). Pozycje wymagające GPU zostają nieruszone razem z H-15
...
- [ ] **7. Decyzje otwarte** — 9.2 (`egui`) i 9.3 (bufor ID) **zostają otwarte** razem
  z H-15; 9.1 wykonana (`K-22`), 9.16 wykonana (bufor śledzenia na 8 × 64 wpisy)
- [~] **Faza ukończona** — artefakt działa bez GPU: `headless m3day` zaludnia miasto M2,
  przewija dobę przez systemy §5.12 i wypisuje kartę inspekcji z osią dnia. Zostaje
  graficzna połowa artefaktu 3 z §1 (pieszy na ekranie, klik, panel)
