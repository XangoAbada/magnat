...
| DG-16 ★ | **Tryb przeglądu: wejście do świata bez postaci** (`ShellAction::Observe`, wiersz „Tylko oglądam" na ekranie wyboru, flaga `--observe`). Decyzja właściciela produktu z 2026-09-18; PRD §13.1 dostał akapit | Świat już to umiał — sesja bez `SetCharacter` ma `player == None`, tyka i buduje karty — brakowało **drogi wejścia**, a jedyną był świat, w którym nikt nie pasuje do predykatu wariantu. Mechanizmem jest **brak komendy**, nie nowy wariant startu: dzięki temu replay odtwarza tryb przeglądu z samej nieobecności `SetCharacter` i nie trzeba go zapisywać ani w kopercie, ani w nagłówku dziennika. Przy okazji: zdanie w pustej zakładce „Dlaczego" mówiło „żadnego z **Twoich** zakładów", a w trybie przeglądu gracz nie ma swoich — jest teraz neutralne („z obserwowanych"), bo śledzenie włącza kliknięcie w zakład, a nie własność |
| DG-17 ★ | **Tryb przeglądu dostaje drogę wejścia w menu głównym** — pozycja „Tryb przeglądu" obok „Nowa gra", ta sama flaga `Shell::observe` co `--observe`. Kreator w tym trybie ma inny tytuł i o jeden wiersz mniej (wariant startu opisuje postać, której nie będzie); ekran wyboru postaci się nie pokazuje. Wiersz „Tylko oglądam" na ekranie wyboru zostaje | `DG-16` dało trybowi wejście, którego w grze nie dało się znaleźć: ekran wyboru postaci stoi **za** generacją świata, więc gracz widzi ten wiersz dopiero po kilkudziesięciu sekundach, po drodze przez trzy ekrany, na których Esc zamykał grę (`V-9` w `M9-gracz-kariera.md`). Menu główne jest miejscem, w którym gracz szuka trybów, a nie po którym je znajduje przypadkiem |
