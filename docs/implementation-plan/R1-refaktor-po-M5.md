...
| 41 | `tools/magnat/src/citizens.rs` | blok `impl Citizens` — 405 linii (ostrzeżenie od 300; **było 450 przed M9a**) | Po przeniesieniu stawiania świata do `game::Session` (`K-68`) zostaje tu jeden temat: spięcie sesji z oknem — wejście `egui`, klatka UI, warstwa Mikro dla renderera i dwa panele. Wszystkie metody sięgają `self.session` i `self.ui`, więc podział wypchnąłby obie przez sygnaturę. Blok **zmalał** w tym commicie i zmaleje dalej, gdy `M9b` przeniesie panele do `game::panels` | `M9b` (rdzeń UI): panele wyprowadzają się z klienta do `game/`, a tutaj zostaje wejście i warstwa Mikro — czyli dwa tematy, a nie jeden o dziesięciu krokach |
| 42 | `sim/economy/src/market/api.rs` | blok `impl Market` — 459 linii (ostrzeżenie od 300; **było 449 przed M9a**) | Trzydzieści kilka odczytów i trzy zapisy nad jednym zamkiem rynku — to jest szew (g) z tabeli „zmiany po M5e": odczyty dla prezentacji i pomiaru. M9a dołożył jeden odczyt (`good_key`, 4 linie), bo komenda gracza niesie **klucz towaru, nie indeks** (00 §5). Podział wzdłuż „odczyt / zapis" jest naturalny, ale przestawia sąsiedztwo metod, które panel woła parami | `M9b` albo `M9e`: panele biznesowe dołożą tu kolejne odczyty i wtedy `api/readout.rs` dostanie własny temat. Próg błędu 500 padnie w tej samej fazie |
