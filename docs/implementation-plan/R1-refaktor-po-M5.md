...
| 9 | `engine/render/src/renderer/frame.rs` | `impl Renderer` — 305 linii (ostrzeżenie od 300) | Pięć linii ponad progiem. Dzielenie pliku o tyle jest kosztem bez zysku | Zniknie samo, gdy pozycja 8 zostanie kiedyś podzielona **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 10 | `engine/render/src/renderer.rs` | `impl Renderer` — 386 linii (ostrzeżenie od 300) | `new` (94) plus `resize` i siedem setterów, z czego `set_overlay` (92) i `upload_far_terrain` (99) to dwie trzecie reszty. Obie wgrywają teksturę i przebudowują grupę wiązań; `renderer/upload.rs` byłby dziś plikiem o dwóch funkcjach bez wspólnego stanu | M2 podpina własne pola pod nakładkę (§6.1). Gdy ścieżek wgrywania będzie więcej niż jedna, `upload.rs` dostanie temat **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 11 | `sim/world/src/population/homes.rs` | `fn dopasuj_mieszkania` — 227 linii (ostrzeżenie od 150) | Lustro stanu, rangi dochód↔wartość, pomiar przedziału median i **pętla poprawkowa 200 tys. prób zamiany** to jeden ciąg: `adres`, `zaloga`, `hist`, `krotszych` i `odchylenie` żyją przez wszystkie cztery etapy i są aktualizowane przyrostowo. Każdy podział wypycha ten stan przez sygnaturę albo przestawia kolejność prób — a ta kolejność **jest częścią hasha** (§3 reguła 1 i ostrzeżenie w §4 R-WP4) | Faza, która zamieni zachłanną wymianę na wyżarzanie. Sufit jest już nazwany komentarzem `ponytail:` nad funkcją; wtedy „pomiar przedziału" i „pętla" rozejdą się same **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 12 | `sim/world/src/population/mod.rs` | `fn generate_population` — 211 linii (ostrzeżenie od 150; **było 280, czyli nad progiem błędu**) | To jest lista dziesięciu kroków i ma prawo być długa — jedyne miejsce, z którego widać całą sekwencję Etapu 8. Dalsze skracanie wymaga wyniesienia kroku 11 (flota, 35 linii), a ten sięga `crate::traffic_build` w ośmiu miejscach i nie ma właściciela w `population/` | Faza dotykająca floty: gdy `traffic_build` dostanie własny „krok 11" jako jedną funkcję, `generate_population` schodzi poniżej 180 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 13 | `sim/world/src/population/mod.rs` | metryka `mod.rs` — 365 linii kodu poza deklaracjami (ostrzeżenie od 300; **było 2138**) | Poza `generate_population` (211) zostają wyłącznie typy wejścia i wyjścia Etapu 8 plus cztery drobiazgi mostu. `population/api.rs` byłby plikiem o czterech typach bez zachowania | Zniknie razem z pozycją 12 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 14 | `sim/world/src/city/sites/place.rs` | plik — 949 linii (ostrzeżenie od 800; §4 szacowało 750) | Jeden ciąg pięciu kroków obsady, w którym stan idzie przez cztery zmienne lokalne `populate`: `kandydaci`, `przydzialy`, `grupa`, `rep`. Kroki 1–4 mutują wszystkie cztery, krok 5 czyta wynik. Podział wypchnąłby czwórkę przez sygnaturę i przestawił moment, w którym rośnie `grupa` — a numer grupy firmy wchodzi do `rng(seed, StreamId::FirmSeed, …)`, czyli do nazw firm i **do hasha** | M6 dokłada tu magazyny i zakłady. Wtedy `place/workplaces.rs` (korekta F1: `rebind_workplaces` + `licz_stanowiska`, ~130 linii, nie dotykają ani `Kand`, ani `Przydzial`) wychodzi za darmo, a reszta schodzi do ~810 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 15 | `sim/world/src/city/sites/closure.rs` | `fn supply_closure_check` — 194 linie (ostrzeżenie od 150; **wartość niezmieniona od dawnego pliku**) | Pięć kroków §5.8 dzielących pięć wektorów stanu (`import`, `demanded`, `osiagalne`, `importowalny`, `budzet_importu`) i sześć przebiegów o stałym budżecie. Wyniesienie KROKU 5 przepuściłoby przez sygnaturę cztery wektory i `&mut [SiteSeed]`, a dowieziony import liczy się **od stanu końcowego**, nie narastająco — rozdzielenie kusi do zwrócenia importu z pętli, czyli do błędu, który komentarz w środku opisuje jako już raz naprawiony | M6d przejmuje funkcję w całości i zamienia stały budżet bram na dynamiczny (wpisane do `M6d-*.md` przez `K-18`). Wtedy „naprawa" i „bilans" rozejdą się same, bo przestaną być jednorazowe **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 16 | `sim/world/src/city/sites/place.rs` | `fn posadz_produkcje` — 168 linii (ostrzeżenie od 150; **wartość niezmieniona**) | Trzy podkroki (3a klastry wg szablonów, 3b reszta, 3c rozluźnienie strefy) czytające i zmniejszające ten sam wektor `zostalo` oraz ten sam `kand`. Kolejność 3a→3b→3c jest kolejnością zajmowania działek, czyli wprost hashem | Ta sama faza co pozycja 14 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 17 | `sim/traffic/src/trip/minute.rs` | `impl TrafficNetwork` — 432 linie (ostrzeżenie od 300; **było 596 w jednym pliku**) | Jeden ciąg życia minuty: `step_minute` → `dispatch` → `advance` → `finish`, w którym `ActiveTrip`, kopiec odjazdów i `stats` są aktualizowane przyrostowo przez wszystkie cztery. Rozcięcie między `dispatch` a `advance` przecina pipeline w połowie, a nie wzdłuż tematu, a kolejność wywołań jest wprost hashem (§3 reguła 2) | M6 dokłada ruch towarowy i rampy. Gdy `dispatch` przestanie być jedną ścieżką (osobne wpuszczanie ciężarówek), `minute/freight.rs` wyjdzie z własnym powodem istnienia **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 18 | `sim/traffic/src/oracle.rs` | `impl TrafficOracle` — 381 linii (ostrzeżenie od 300; **było 774**) | Po wyjęciu ofert i podróży zostaje `new` plus ~25 getterów i setterów oraz cztery zapytania trasujące, wszystkie sięgające tych samych zamków (`parking`, `transit`, `router`). `oracle/routing.rs` byłby dziś plikiem o czterech funkcjach dzielących stan z resztą przez `self` | M4/M6 przy `TravelTimeMatrix` per dzielnica × godzina: wtedy trasowanie dostanie własny stan i własny temat **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 19 | `sim/traffic/src/oracle/offers.rs` | `impl TrafficOracle` — 363 linie (ostrzeżenie od 300) | Pięć wariantów jednego kontraktu plus `plan`, które je porównuje. Dalszy podział albo rozdziela warianty po plikach — niszcząc możliwość porównania ich wzrokiem, czyli **cel** podziału z §4 — albo sprowadza je do wspólnej abstrakcji, czego §4 zakazuje wprost | M4c+/M6: gdy dojdzie szósty i siódmy środek (rower miejski, dostawa), podział `offers/individual.rs` i `offers/shared.rs` wyjdzie sam **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 20 | `sim/traffic/src/oracle/journey.rs` | `fn start_trip` — 167 linii (ostrzeżenie od 150; **wartość niezmieniona**) | Jeden ciąg: decyzja → rezerwacja pojazdu → trasa (z ewentualnym postojem na stacji) → `PendingTrip` → wpis do kolejki. Każdy wyniesiony fragment przepuszcza `ModeDecision`, `TripRequest` i `RouteQuery` przez sygnaturę | Faza, która rozdzieli tankowanie od wyruszenia — M6, gdy stacja paliw stanie się zakładem z zapasem **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 21 | `sim/agents/src/demography/mod.rs` | metryka `mod.rs` — 495 linii kodu poza deklaracjami (ostrzeżenie od 300; **było 1871 w jednym pliku**) | Poza typami wejścia i wyjścia doby oraz miesiąca (`Population` 140 linii, `LifeQueue` 99, dwa raporty, osiem kluczy `K_*`) zostaje tu piątka relacyjna (98 linii), która **nie ma dokąd pójść** — powód w `D-16`. Osobny `relations.rs` zamieniłby jedną krawędź zależności na pięć i zostawiłby `mod.rs` na 397 liniach, czyli nadal nad progiem ostrzeżenia | Faza, w której narodziny i zgon przestaną same pisać relacje rodzinne. Wtedy `demography` przestaje wołać `powiaz`, kierunek `social → demography` zostaje jednostronny **także po przeniesieniu**, piątka jedzie do `social.rs` tak, jak §4 R-WP7 zakładał, a `Population` i `LifeQueue` dostają przy okazji powód, żeby wyjść do `demography/registry.rs` **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 22 | `sim/world/src/city/build.rs` | `fn plan_building_inner` — 160 linii (ostrzeżenie od 150; **wartość niezmieniona od dawnego pliku**) | Jeden ciąg: filtr strefy → oś frontu i pierzeja → rozpiętość działki → `PickCtx` → `pick_grammar` → `footprint_for` → niwelacja → trzy pomiary `zapas` → `derive`. Każdy wyniesiony fragment przepuszcza przez sygnaturę `PickCtx`, `Scope`, parę `(lo, hi)` i marginesy, a kolejność losowań ze strumienia `StreamId::BuildingPick` **jest hashem** — `pick_grammar` losuje raz na cały dobór właśnie po to, żeby działka po drugim podejściu nie przesunęła strumienia sąsiadce | M6 dokłada tu zakłady i magazyny. Jeśli wtedy powstanie drugi **temat**, a nie drugi krok tego samego, szew będzie widać **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 23 | `sim/world/src/city/build/capacity.rs` + `build/interiors.rs` | cztery liczby kalibracyjne bez nazwy (`1.025` — środek okna T10, czyli **cel kalibracji**; `12.0` — faktyczny twardy sufit `GESTOSC_MAX`; `0.02` — próg „nie ma czego kalibrować") i **jedna zduplikowana**: `28.0` m² na pokój żyje w `rescale_dwellings` **i** w `rodzaj` | §2 R1: ani jednej zmiany zachowania. Nazwanie stałej jest darmowe, ale scalenie duplikatu `28.0` zmienia liczbę miejsc, w których reguła domenowa żyje, i jest pracą właściciela kalibracji, nie refaktoru. Podział to **uwidocznił** — obie kopie wylądowały w różnych plikach | M5e (balansator): to on ma powód, żeby te liczby ruszyć, i on pierwszy zobaczy, że jedna jest w dwóch egzemplarzach. Jeśli ruszy tylko jedną, sylwetka mieszkań rozjedzie się z metrażem **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 24 | `sim/world/src/city/mod.rs` | `fn generate_city` — **370** linii (błąd od 250; 365 do M6d, +3 w M6e za bilans otwarcia złóż `AP-1`, +2 w M11c za chodniki przy parcelach `R2-WP15`) | To jest lista etapów 3–7 i ma prawo być długa, jak `generate_population` (pozycja 12) — **ale 147 z 365 linii to nie etapy**: 76 linii składania `warnings` i 71 linii literału `GenerationReport { … }`. Wyniesienie pierwszego przepuszcza przez sygnaturę siedem struktur plus `bez_frontu`, czytane też przez drugie; wyniesienie drugiego to konstruktor o ~20 parametrach. To jest **zmiana kształtu, nie przeniesienie bloku**, a konstruktor powołany po to, żeby zbić metrykę, jest gorszy od metryki | **M6a**: ta faza dokłada do `GenerationReport` sekcję łańcucha dostaw, więc `report::zbierz(…)` dostaje wtedy powód istnienia inny niż liczba linii. `generate_city` schodzi do ~290, a wyniesienie `warnings` w tej samej fazie — poniżej 220 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 25 | `sim/agents/src/planner/mod.rs` | metryka `mod.rs` — **366** linii kodu poza deklaracjami (ostrzeżenie od 300; **było 1772 w jednym pliku**) | Poza czterema typami kontraktu i dwoma orkiestratorami zostaje tu `Log` (41 linii), który **nie ma dokąd pójść**: pisze do niego każda z czterech faz, więc `planner/reasons.rs` zamieniłby jedną krawędź zależności na sześć i zostawiłby `mod.rs` na 325 liniach, czyli nadal nad progiem ostrzeżenia. Dokładnie ten sam układ co piątka relacyjna w `demography/mod.rs` (pozycja 21) | Faza, w której uzasadnienia przestaną być odtwarzane synchronicznie z planem. Wtedy `Log` przestaje być cieniem `DayCanvas`, trójka `ReasonLog` + `Log` + `ReasonEntry` (90 linii) dostaje własny powód istnienia, a `mod.rs` schodzi do ~276 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 26 | `sim/economy/src/market/fulfil.rs` | `fn PlaceProvider::candidates` — **258** linii (błąd od 250; **wartość niezmieniona od dawnego pliku**) | **Stan zastany, nie skutek podziału** (na `master` ten sam błąd, linie 2696–2953). Zejście poniżej 250 wymaga wycięcia bloku, a każdy sensowny blok przecina ciąg `offer_buf.clear()/extend` → sortowanie `cand_buf` → softmax `util_buf` → `choose_offer`, czyli **jedyne miejsce, w którym ustala się kolejność sumowania w softmaksie (`K-6`)** — wprost objęte zakazem z §4 R-WP12 | Faza, która rozstrzygnie, czy zbieranie ofert w zasięgu (pierwsze ~60 linii, przed pierwszym sortowaniem) da się wyjąć jako osobną funkcję **bez przesunięcia `clear()`**. Dopóki nie wiadomo, że tak, cięcie jest ryzykiem bez zysku — **Wykonane w M6d/WP11.** Odczyt półki z magazynu (`shelf_units`, `shelf_pick`) przepchnął funkcję z 254 na ponad twardy próg, więc pytanie „czy da się wyjąć zbieranie ofert bez przesunięcia `clear()`" trzeba było zadać na serio. Odpowiedź: **da się**, i to bez dotykania ciągu, o który chodziło. `zbierz_kandydatow` bierze oferty w zasięgu, odsiewa nieznane i za małe i zamienia je w kandydatów; `clear()` buforów, sortowanie, softmax i `choose_offer` zostają w `candidates` w niezmienionej kolejności. 254 → 226 linii, zero zmian zachowania (hashe `m5shop` zmieniły się z powodu migracji WP11, nie z powodu podziału) **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 27 | `sim/economy/src/market/fulfil.rs` | `fn Market::fulfil` — 218 linii (ostrzeżenie od 150; wartość niezmieniona) | Jedna ścieżka decyzyjna z ośmioma powodami odmowy. Rozcięcie jej znaczy rozcięcie `FulfilOutcome`, czyli zmianę kontraktu, a nie przeniesienie bloku | Ta sama faza co pozycja 26 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 28 ✅ | `sim/economy/src/market/readout.rs` | `impl Market` — 313 linii (ostrzeżenie od 300) | Dwie metody **czysto odczytowe**, po ~150 linii każda (`balance_sample`, `shop_panel`). Podział na dwa pliki po 160 linii dokłada krawędź importową bez zysku — a to jest jedyny szew `market.rs`, o którym wiadomo, że nie może zmienić hasha nawet przy błędzie implementacji | M5e/M9: gdy panel dostanie drugą zakładkę czytającą inne pola, `readout/` dostanie temat ✅ **Zamknięta w `R2-WP26` (2026-09-22): pomiar `struct_guard` nie widzi już tego przekroczenia.**| — |
| 29 | `sim/economy/src/market.rs` | plik — **803** linie rodzica (przekroczyło 800 w M10e jednym polem `ShelfSnapshot::price_gross`; 571 po R1) (poniżej progu ostrzeżenia 800, ale powyżej liczby 500 z kryterium §4 R-WP12) | Rodzic jest właścicielem stanu: `MarketInner` z trzydziestoma polami, `new`, `lock`, akcesory ticka, migawka kupującego i `impl HashState`. Sam nagłówek modułu (39), blok `use` (43), definicje typów (216) i `HashState` (48) to 346 linii, zanim dojdzie cokolwiek wykonawczego — liczba 500 z §4 była przy tym przydziale treści **nieosiągalna** | M6 dopisuje tu rynek B2B. Jeśli plik przekroczy 800, następnym szwem jest wyniesienie definicji typów (`PurchaseIntent`, `ShopSeed`, `Bank`, `HouseholdMonth*`, `MarketStats` — 216 linii) do `market/types.rs`, co zostawia rodzicowi ~355 linii samego stanu **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 30 | `sim/traffic/src/transit/sim.rs` | `impl TransitNetwork` — 360 linii (ostrzeżenie od 300; **było 689 w jednym pliku**) | Jeden ciąg życia minuty `step_minute` → `dispatch` → `advance` → `give_up`, ten sam kształt co pozycja 17 (`trip/minute.rs`). `przesiadka` i `give_up` dzielą z `advance` kolejkę oczekujących i licznik `stats` | Zniknie razem z pozycją 31 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 31 | `sim/traffic/src/transit/sim.rs` | `fn advance` — 176 linii (ostrzeżenie od 150; **wartość niezmieniona od dawnego pliku**) | Cztery etapy przystanku w jednej pętli po kursach, z reborrowami `&mut self.runs[ri]` trzymanymi **przez granice etapów**, `mem::take` na kolejce w środku etapu 2 i `swap_remove` po pętli w odwrotnej kolejności. Wyjęcie etapu ponownie pobiera każdy reborrow w innym punkcie — to **przepisanie struktury pożyczek, nie przeniesienie bloku** (§3 reguła 2). §4 R-WP10 dopuszcza podział tylko wtedy, gdy kolejność zdarzeń przeżyje; nie przeżywa | M6 dokłada ruch towarowy i rampy. Jeśli etap *wsiadka* doczeka się drugiego wołającego, wyjęcie go stanie się przeniesieniem, a nie przepisaniem **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 32 | `sim/world/src/city/build.rs` | martwy re-eksport `pub use model::{… ShiftKey …}` — zero konsumentów w workspace | `clippy::upper_case_acronyms` na wariancie `III` milczy **wyłącznie dla API eksportowanego** (`avoid-breaking-exported-api = true` w `clippy.toml`). Usunięcie re-eksportu budzi lint, a uciszenie go wymaga `#[allow]`, czyli zmiany kodu — zakazanej w R1 §2. Wybór był między dwoma złami i padł na mniejsze | Dowolny commit spoza R1: `#[allow(clippy::upper_case_acronyms)]` nad `ShiftKey` albo przemianowanie wariantu `III`. Wpisane, bo bez tego wyjaśnienia wygląda na przeoczenie R-WP10 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 33 ✅ | `sim/world/src/city/lsystem.rs` | `impl<'a> Builder<'a>` — **609** linii (błąd od 500; +4 w M11c za warunkowe nadanie `RoadFlags::SIDEWALK`) | **Jedyna z ośmiu pozycji, którą da się podzielić dziś i za darmo.** Szew po linii 422: do niej indeks i reguły terenowe (`cell`…`segments_at_node`, `forbidden`…`angle_ok`), od niej wzrost (`pattern_at`, `steer`, `grow`). Obie strony to przeniesienie bloku — potomek widzi prywatne pola `Builder` za darmo (`D-8`), więc zero `pub(crate)`, zero zmian sygnatur, zero zmian kolejności. Brakowało tylko pakietu w §4: plik ma 1154 linie, czyli **poniżej progu błędu dla metryki *plik***, i przez to nie trafił na listę ośmiu z §1 | Pakiet w `R2`, albo commit spoza R1. 257 z 605 linii to `grow` (pozycja 35) — samo jego wyniesienie zbija blok do 348, czyli do ostrzeżenia ✅ **Zamknięta w `R2-WP26` (2026-09-22): pomiar `struct_guard` nie widzi już tego przekroczenia.**| — |
| 34 ✅ | `sim/world/src/city/lsystem.rs` | `fn grow_network` — **317** linii (błąd od 250) | Cała pętla L-systemu: aksjomat z bram, zalążek obwodnicy, kopiec propozycji i pięć reguł produkcji P0–P4 na wspólnym `props`/`heap`/`seq`/`b`. Szew po ~868 (zasiew / pętla) wymaga przepuszczenia przez sygnaturę czwórki `&mut` i zamiany domknięcia `push` na funkcję wolną; wewnątrz pętli szwu nie ma wcale. **`seq` jest licznikiem kolejności wepchnięć i zarazem kluczem losowania** `rng(seed, StreamId::RoadsL, prop.seq, …)` — każde przestawienie `push` przesuwa strumień wszystkim następnym propozycjom | Faza, która otworzy generator dróg. **Dziś nie planuje go żadna z M6–M12** — sprawdzone: M8e trzyma politykę drogową na krawędziach grafu M4, M10a zakłada drogi Etapów 3–5 jako gotowe, M12d wymienia tylko `city/grammar.rs` ✅ **Zamknięta w `R2-WP26` (2026-09-22): pomiar `struct_guard` nie widzi już tego przekroczenia.**| — |
| 35 | `sim/world/src/city/lsystem/constrain.rs` | `fn Builder::grow` — **257** linii (błąd od 250) | Próba wyprowadzenia jednego segmentu, cztery etapy na sześciu zmiennych lokalnych, które każdy etap nadpisuje. Komentarz w środku mówi, dlaczego etap 3 istnieje: *snapowanie, przedłużenie i podział — każde z nich zmienia koniec odcinka*. Do tego `self.stats.rejected_*` przeplecione z siedmioma `return None`, więc wyniesienie fragmentu przesuwa moment, w którym licznik rośnie | Ta sama faza co pozycja 34. **Wyniesienie `grow` kasuje pozycję 33, ale nie kasuje tej** — 257 linii zostaje 257 liniami w nowym pliku **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 36 | `sim/world/src/city/zoning.rs` | `fn assign_zones` — **322** linie (błąd od 250) | Cały WP7 strefowania w sześciu ponumerowanych krokach na wspólnej macierzy `score[n][16]`. Najczystszy jest krok 6 (bufor przemysłu ciężkiego, ~1010–1080) — przeniesienie bloku plus sygnatura na pięć argumentów; kroki 1–4 są splecione (krok 4 czyta macierz z 1 i kwoty z 3). **Rng nie ma tu wcale**; determinizm stoi na `sort_by(total_cmp)` z jawnymi remisami po `BlockId` i na kolejności kroków | ~~**M8e**~~ → **bez właściciela fazowego (sprostowanie po M8e, `CI-1`)**. Wyzwalacz nie zadziałał i tym razem nie dlatego, że przebudowa była za droga, tylko dlatego, że **`Policy::Zoning` nie powstał**: `Parcel.zone` czyta w całym repozytorium wyłącznie generator miasta (`city/build.rs`, `city/districts.rs`), a generator biegnie raz, przed pierwszym tickiem. Uchwała o przekwalifikowaniu parceli zmieniałaby pole, którego nikt już nie przeczyta — czyli byłaby wariantem bez skutku (`R2`). Ścieżki *przekwalifikuj jedną parcelę* potrzebuje dopiero faza, w której zabudowa reaguje na strefę **w trakcie gry**: rynek nieruchomości (M10) albo przebudowa miasta (M12). Do tego czasu pozycja zostaje w rejestrze jako dług bez wyzwalacza, a nie jako zadanie z adresem, który się nie sprawdził | N8.3 · przegląd 2026-10-22 |
| 37 | `engine/ui/src/inspect/reason/firm.rs` | `fn describe` — **618** linii (błąd od 250; 319 przed M6b, 355 po M6b, 451 po M7b, 566 po M7d) | Tabela `match`: jedno ramię na wariant `DecisionReason`, zero logiki, zero stanu, ma już `#[allow(clippy::too_many_lines)]`. Szew tematyczny istnieje, ale **`DecisionReason` jest enumem płaskim**, więc podział daje albo powtórzenie listy wariantów we wzorcach (dwa miejsca do zapomnienia zamiast jednego), albo pomocnicze funkcje zwracające `Option<String>` — a to **kasuje wyczerpującość sprawdzaną przez kompilator**, czyli jedyną rzecz, która gwarantuje, że nowy wariant nie przejdzie bez tekstu | ~~**M7c**~~ → **osobna zmiana** (patrz sprostowanie w opisie). Przebudowa `DecisionReason` na `Citizen \| Firm \| City` z `M7c-polityki-i-menedzerowie.md` §5.11 zostaje jedyną drogą wyjścia; po tym `describe` rozpada się **z konstrukcji**, a każdy podzbiór zachowuje własną wyczerpująceść. Uwaga: **M6 dopisuje tu wcześniej sześć wariantów** (`Shortage`, `SupplierChosen`, `ContractSigned`, `ExportChosen`, `SubstituteUsed`, `ProductionHalted`), więc funkcja najpierw rośnie o ~80 linii. **Wykonane:** M6b dołożył trzy (355), M6c pozostałe trzy (397) — prognoza się sprawdziła, blok M6 jest zamknięty. **M7b dołożył blok M7** (`Hired`, `WageRaise`, `JobLeft`) i **451**: pięć ramion na trzy warianty, bo `WageRaise` z przyrostem zerowym jest innym zdaniem („stawka bez zmian, sufit marży") niż podwyżka o zero, a `Hired` bez drugiego kandydata innym niż `Hired` z remisem. **Sprostowanie po M7c: wyzwalacz nie zadziałał i pozycja zostaje bez właściciela fazowego.** M7c dołożył `PolicyApplied` i `ManagerAssigned` (znów trzy ramiona na dwa warianty, bo reguła zapasowa jest innym zdaniem niż reguła numer 255) i funkcja ma **499** linii — ale **przebudowy `DecisionReason` nie zrobił**. Powód jest zmierzony, nie estetyczny: przebudowa dotyka **370 miejsc w dziesięciu crate'ach**, zmienia reprezentację w zapisie gry i pakowanie `PlanSlot.reason` do bajtu, a do żadnego z dwóch kryteriów M7c nie wnosi nic. Wpuszczona do commita podfazy zatopiłaby jego recenzję dokładnie tak, jak `cargo fmt` całego repo zatopiłby recenzję M7a. **Rozstrzygnięcie stoi:** podział `describe` nadal wymaga tamtej przebudowy i nadal jest właściwym rozwiązaniem; zmienia się wyłącznie to, kto ją niesie — **osobna zmiana, nie podfaza**, bo przebudowa kontraktu z 00 §K-12 nie jest pakietem roboczym żadnej fazy. Do tego czasu funkcja rośnie o ~20 linii na podfazę M7 i jest zamrożona w `struct_guard.py` na aktualnej wartości. **Po M7d: 566.** Blok finansowy (505–510) dołożył siedem ramion na sześć wariantów — `BankruptcyOpened` ma dwa, bo brak płynności mierzy się dobami („nie płaci od trzech miesięcy"), a ujemny kapitał nie („ma więcej długów niż majątku"), i to są dwa różne zdania o firmie. Prognoza „~20 linii na podfazę" okazała się zaniżona **dwukrotnie i konsekwentnie**: M7b +54, M7c +48, M7d +67. Powód jest ten sam za każdym razem i warto go zapisać, bo zmienia szacunek dla M8–M10: wariant z ładunkiem złożonym rzadko ma jedno ramię, a formatowanie argumentów zajmuje pięć linii na ramię, nie jedną. **Po M7e: 618.** Pięć wariantów AI firm (511–515), pięć ramion — pierwszy blok M7, w którym żaden wariant nie rozgałęził się na dwa klucze lokalizacji, i pierwszy, w którym przyrost (+52) trafił w prognozę. **Po M7f: 660** i blok M7 jest zamknięty (516–519: `SiteOpened`, `VoluntaryClosure`, `FirmFounded`, `ChainEntered`). Przyrost +42 jest najmniejszy w całej fazie i pokazuje, gdzie naprawdę leży koszt tej funkcji: `SiteOpened` niesie `Trend` o trzech wartościach i ma **jedno** ramię, bo kierunek wchodzi podstawieniem do zdania, a nie wyborem klucza. Ramię kosztuje pięć linii; **rozgałęzienie na drugi klucz kosztuje kolejne pięć** — i to ono, a nie liczba wariantów, decydowało o przyrostach M7b (+54) i M7d (+67). Adres podziału zostaje bez zmian: osobna zmiana przebudowująca `DecisionReason` na `Citizen \| Firm \| City`. **Po M8a: 730.** Blok M8 (600–606) dołożył siedem ramion na siedem wariantów — znów żadnego rozgałęzienia na drugi klucz, ale przyrost wyszedł **+70**, największy od M7d. Powód doprecyzowuje wcześniejszy wniosek: nie liczy się liczba ramion ani liczba kluczy, tylko **liczba podstawień w ramieniu**. Każdy powód podatkowy niesie trzy (danina, kwota, stawka albo przyczyna), a trzy argumenty to dziesięć linii formatowania, nie pięć. Gdyby siedem danin dostało po własnym zdaniu zamiast jednego zdania z podstawieniem `{danina}`, ta sama treść zajęłaby ~250 linii — i to jest miara tego, ile ten plik zawdzięcza podstawieniom **Po M8b: 754** (607–608, sieci przesyłowe, +24 — przyrost wraca do prognozy, bo rodzaj medium wchodzi podstawieniem `{medium}`, tak samo jak danina). **Po M8c: 780** (609–610, zdarzenia świata, +26 — sześć kategorii wchodzi jednym `{kategoria}`). **Po M8d: 855**, największy przyrost bloku M8 (+75) z dwóch powodów naraz: `ServiceQuality` ma **sześć** podstawień, bo jakość placówki bez rozbicia na czynniki jest liczbą bez odpowiedzi na „dlaczego tyle”, a `RemedyImposed` jest **pierwszym ramieniem bloku M8 rozgałęzionym na dwa klucze lokalizacji** — kara z kwotą i kara bez kwoty to dwa różne zdania. **Po M8e: 972 (+117) i po raz pierwszy przekroczony próg pliku (1329 > 1200).** Siedem wariantów władzy i wyborów (616–622), z czego **trzy rozgałęziają się na dwa klucze**: podwyżka stawki i obniżka, przetarg rozstrzygnięty i przetarg bez ofert, wybory utrzymujące burmistrza i wybory zmieniające władzę. To domyka regułę zbierającą całą historię tej pozycji: **koszt bierze się z rozgałęzień, nie z wariantów** — M7f (+42, zero rozgałęzień) i M8e (+117, trzy) są jej dwoma końcami. Przekroczenie progu pliku niczego nie zmienia w adresie podziału: plik ma **jeden temat** (powód → zdanie), więc rozcięcie go dałoby dwa pliki o jednym temacie. Rozcina go dopiero `K-58` z `R2e` — rozbicie `DecisionReason` na trzy enumy po aktorze — i to jest ten sam podział widziany z drugiej strony. **Po M9d: 978 (+6) przy pliku 1360 (+31).** Pierwszy przyrost, który **nie pochodzi z nowego wariantu**: M9d dołożył dwa pola do istniejącego `PolicyApplied` (wiek danych i odchyłka menedżera) i wynikające z nich zdanie warunkowe. Sześć linii w funkcji, trzydzieści w pliku — bo dopisek o ręce menedżera jest **osobną funkcją** (`reka_menedzera`), a nie trzecim ramieniem. To jest zarazem tania odpowiedź na pytanie, które ta pozycja zadaje od M6b: **podstawienie zamiast rozgałęzienia kosztuje sześć linii, a rozgałęzienie kosztowałoby czterdzieści**, bo menedżer doskonały i menedżer słaby musieliby mieć po dwa klucze razy dwa ramiona `PolicyApplied`. **Po M10b: 1047 (+69) przy pliku 1458 (+98).** Cztery warianty marki i mediów (800–803), z czego **jeden rozgałęziony** (`BrandExperience`: „zawiodła" i „spełniła oczekiwania" to dwa różne zdania o tej samej liczbie), plus **trzy nowe funkcje pomocnicze** nazywające słowniki (`touch_source`, `ad_channel`, `editorial_bias`) i jedna nazywająca markę. Przyrost trafia w regułę tej pozycji co do linii: jedno rozgałęzienie plus dziesięć podstawień. Nowe jest co innego i warto to zapisać przed M10c: **funkcja przekroczyła tysiąc linii, czyli czterokrotność progu błędu**, a blok M10 ma jeszcze cztery podfazy (R&D, giełda, relacje, związki, kroniki). Przy dotychczasowym tempie (+70 na podfazę) zamknięcie M10 zostawi ją w okolicy 1300. Adres podziału zostaje **bez zmian i bez właściciela fazowego**: `K-58` z `R2e`, rozbicie `DecisionReason` na trzy enumy po aktorze. Wniosek praktyczny dla tego, kto po nią sięgnie: koszt przebudowy rośnie liniowo z każdą podfazą, a korzyść nie — więc jeśli ma się zdarzyć przed M12, to lepiej przed M10c niż po M10f. **Po M10c: 1106 (+59) przy pliku 1527 (+69).** Cztery powody R&D (804–807), z czego **jeden rozgałęziony**: odkrycie z patentem i odkrycie po roku „światowym” to dwa różne zdania o tym samym wyniku, bo różnicę robi wyprzedzenie świata, a nie znak liczby. Plus jedna funkcja nazywająca technologię. Przyrost trafia w regułę tej pozycji co do linii i **jest pierwszym, który wypadł poniżej prognozy** (+70 na podfazę): powodem jest to, że trzy z czterech wariantów niosą po dwa–trzy podstawienia, a nie po sześć jak `ServiceQuality` z M8d. Rada sprzed M10c — „jeśli przebudowa ma się zdarzyć, to raczej przed M10c niż po M10f” — **nie została wykonana i nie jest już aktualna w tej formie**: zostały trzy podfazy, więc próg opłacalności przesuwa się na „po M10f, przed M12”. **Po M10d: 1190 (+84) przy pliku 1638 (+111).** Dziewięć powodów giełdy i ubezpieczeń (808–816), z czego **dwa rozgałęzione**: `StakeDisclosed` i `ControlAcquired` wybierają inny klucz dla posiadacza-osoby i posiadacza-firmy, bo po polsku różnią się rodzajem czasownika, a po angielsku rzeczownikiem. Plus dwie funkcje pomocnicze (`peril_kind`, `podmiot`) i jedna licząca wycenę firmy z kursu. Przyrost jest **drugi co do wielkości w całym bloku M10** i potwierdza regułę tej pozycji trzeci raz z rzędu: liczy się liczba rozgałęzień, nie wariantów. **Po M10e: 1276 (+86) przy pliku 1724 (+86).** Osiem powodów relacji, zmów i związków (817–824), z czego **jeden rozgałęziony**: `StrikeEnded` wybiera inny klucz dla strajku wygranego i przegranego, bo zero podwyżki znaczy kapitulację, a nie brak pomiaru — i to są dwa różne zdania o tej samej liczbie dób. Zero nowych funkcji pomocniczych, bo wszystkie słowniki, których te powody dotykają (marka, procent), miały już swoje. Reguła tej pozycji trzyma się piąty raz z rzędu. Została **jedna** podfaza fazy M10 **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.** | N8.3 · przegląd 2026-10-22 |
| 38 | `sim/economy/src/data.rs` | `fn EconomyData::load` — **269** linii (błąd od 250) | Siedmiokrotnie powtórzony ten sam rytuał (odczyt → `schema_version` → duplikaty → kompletność) zakończony literalem o ~20 polach. **Najtańszy szew z całej ósemki**: pięć z siedmiu bloków zwraca typ, który **już istnieje** (`RetailTable`, `(PricingParams, ShopCosts)`, `BudgetParams`, `BankParams`, `CpiSpec`), więc ich wyniesienie to przeniesienie bloku z sygnaturą `fn(dir: &Path) -> Result<T, EconomyDataError>`. Zostają dwa: `weights.ron` i `choice.ron`, który rozsypuje dziewięć skalarów wprost do literału — ten wymagałby typu pośredniego. Po wyniesieniu piątki `load` schodzi do ~110 | **M6a** (`M6a-katalog-i-partia.md` WP1): wprowadza `data/goods/`, `GoodId` alfabetyczny i `pack: Option<RetailPack>` opisany jako kontrakt z M5 — czyli moment, w którym `retail.ron` dostaje ósmy czytnik. Kolejność `goods` w `RetailTable` jest kontraktem danych (ranga substytutu), więc blok musi pojechać z zachowaniem kolejności wierszy **Sprostowanie po M6a:** wyzwalacz nie zadziałał i pozycja przechodzi do **M6d/WP11**. M6a nie dołożył `pack: Option<RetailPack>` do `Good` — pole nie ma w M6 ani jednego konsumenta i zostało przeniesione do WP11 razem z podłączeniem półki (`AD-4` w dokumencie fazy M6). `retail.ron` dostał w M6a wyłącznie przemianowane klucze, czyli ani jednego nowego czytnika, więc `load` ma te same 268 linii co przed fazą — **Sprostowanie po M6d:** wyzwalacz nie zadziałał **drugi raz** i pozycja przechodzi do **M6e**. WP11 zmienił, **gdzie leży towar**, a nie schemat danych: `retail.ron` nie dostał ani jednego nowego czytnika, `data.rs` nie jest w diffie tej podfazy, a `Good::pack` z `AD-4` okazał się niepotrzebny (`AL-10`). Dzielenie pliku, którego zmiana nie dotyka, w commicie o pięćdziesięciu plikach, jest dokładnie tym rodzajem przejazdu przy okazji, przez który commit przestaje dać się przeczytać — a `load` ma te same 268 linii co przed fazą. Adres na przyszłość bez zmian: pierwsza faza, która dopisze czytnik do `retail.ron` (M6e panel, M7 asortyment). **Sprostowanie po M7d: wyzwalacz nie zadziałał trzeci raz, a funkcja urosła o jedną linię (269).** M7d dopisał do `bank.ron` sekcję `finance` (leasing, faktoring, obligacja) i produkt inwestycyjny, ale to jest **nowe pole w istniejącym bloku**, a nie nowy czytnik: `BankParams` buduje się dokładnie tam, gdzie się budował, o jeden wiersz dłużej. Podział pliku, którego zmiana dotyczy w jednej linii, w commicie zamykającym podfazę, jest tym samym przejazdem przy okazji, który odrzuciło M6d **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 39 | `sim/economy/src/market/lifecycle.rs` | `impl Market` — 367 linii (ostrzeżenie od 300; 361 przed M9e), w tym `fn open_shop` 192 (ostrzeżenie od 150) | **Urosło w M6d/WP11 i urosło we właściwym miejscu.** Otwarcie sklepu zakłada teraz dwa sloty magazynowe, rampę i zakład w `Plant` — to jest ta sama czynność co przedtem (postawienie sklepu), tylko wykonana do końca. Szew tematyczny **istnieje**: `open_shop` + `stock_initial` (założenie i zatowarowanie) kontra `record_capital` + `deliver_now` (pieniądz i dostawa z zewnątrz), i przecina plik mniej więcej na pół. Nie jest jednak mechaniczny: obie połowy sięgają do `ChainHandle` i do `Books`, więc podział w tym samym commicie co migracja znaczyłby dwie zmiany naraz w pliku, którego cały sens się właśnie zmienił | **M7a** (`M7a-firma-jako-dane.md`): zakładanie firmy przestaje być czynnością scenariusza i staje się decyzją AI, więc `open_shop` i tak dostaje drugiego wołającego z innymi wymaganiami — a wtedy podział ma po co się wydarzyć, zamiast być przestawieniem linii **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 40 | `engine/core/src/vocab.rs` | plik **802** linie (ostrzeżenie od 800, błąd od 1200) | Słownik wspólny wszystkich faz i **z definicji rosnący**: każde wykonanie `K-8` dokłada tu enum. Po M7f przekroczył próg ostrzeżenia dopisaniem `Trend` (`K-52`). Podział jest oczywisty tematycznie — słowniki agenta, gospodarki, terenu i czasu — ale kosztuje **przenumerowanie nazw w `use` w dziesięciu crate'ach** i nic nie kupuje, dopóki plik mieści się w progu błędu. Ścieżka wyjścia: `vocab/` jako katalog z re-eksportem w `mod.rs`, czyli zmiana bez zmiany ani jednej nazwy publicznej | **M8** — pierwsza faza, która dopisze tu więcej niż jeden słownik naraz (podatki, prawo pracy, usługi miejskie), i pierwsza, przy której próg błędu przestanie być odległy **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 41 | `tools/magnat/src/citizens.rs` | blok `impl Citizens` — **493** linie (ostrzeżenie od 300; 450 przed M9a, 405 po M9b, 448 po M9c) | Po przeniesieniu stawiania świata do `game::Session` (`K-68`) zostaje tu jeden temat: spięcie sesji z oknem — wejście `egui`, klatka UI, warstwa Mikro dla renderera i dwa panele. Wszystkie metody sięgają `self.session` i `self.ui`, więc podział wypchnąłby obie przez sygnaturę. Blok **zmalał** w tym commicie i zmaleje dalej, gdy `M9b` przeniesie panele do `game::panels` | **Sprostowanie po M9c: prognoza się nie sprawdziła i warto wiedzieć dlaczego.** `M9b` miał wyprowadzić panele, a `M9c` zamiast tego **zastąpił dwa panele jednym** (karta inspekcji zamiast karty mieszkańca i karty sklepu) i dołożył do klienta nawigację po kartach, filtr encji i legendę nakładki. Netto +43 linie: treść kart przeniosła się do `game::inspect`, ale ich **obsługa** została tutaj i tu zostanie. Adres podziału zmienia się na `M9e`: panele biznesowe dostaną własny rejestr (`PanelRegistry`), a wtedy z tego bloku wychodzi „wejście i warstwa Mikro" osobno od „panele i karty" — czyli wreszcie dwa tematy **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 42 | `sim/economy/src/market/api.rs` | blok `impl Market` — **499** linii (ostrzeżenie od 300, **jedna linia od progu błędu 500**; 449 przed M9a, 459 po M9a, 498 po M9c) | Trzydzieści kilka odczytów i trzy zapisy nad jednym zamkiem rynku — to jest szew (g) z tabeli „zmiany po M5e": odczyty dla prezentacji i pomiaru. M9a dołożył jeden odczyt (`good_key`, 4 linie), bo komenda gracza niesie **klucz towaru, nie indeks** (00 §5). Podział wzdłuż „odczyt / zapis" jest naturalny, ale przestawia sąsiedztwo metod, które panel woła parami | **M9c dołożył dwa odczyty** (`lost_sales_of_citizen` i `is_customer`, razem ~39 linii): karta mieszkańca pyta o utracone sprzedaże **jego**, a filtr encji o to, czy kupił u gracza. Próg błędu 500 padnie przy pierwszym następnym odczycie, więc **podział jest teraz zaplanowany na `M9e`, a nie „kiedyś"**: panele biznesowe dołożą ich kilkanaście, a szew („odczyt dla prezentacji" → `api/readout.rs`) jest ten sam, który był naturalny już w R1  **Po M10e jedna linia od błędu**: zmowa cenowa potrzebowała ceny **brutto** w migawce półek, a `price_net` już tam było — dołożenie `price_gross` obok kosztowało jedną linię w `shelf_snapshot`. Kto dotknie tego pliku jako następny, przekroczy próg błędu i będzie musiał go podzielić; szew jest ten sam co wyżej i nie zmienił się od M9a. **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 43 ✅ | `engine/core/src/decision.rs` | plik **1067** linii (ostrzeżenie od 800, błąd od 1200; 814 po M9a, 1000 po M10d) | To jest jedna lista: centralny `DecisionReason` ze wszystkimi wariantami wszystkich faz plus jawny `discriminant`. Podział pliku rozbiłby listę, której jedyną wartością jest to, że **widać ją w całości** — numer wolny znajduje się wzrokiem, a nie wyszukiwarką po trzech plikach. Wartość wyszła na jaw przy `cargo fmt --all` w M9a (formatowanie skróciło plik, kontrola strukturalna patrzy na pliki zmienione); przed formatowaniem plik był dłuższy i nigdy nie był zarejestrowany | `R2e` dzieli `DecisionReason` na trzy enumy po aktorze (`Citizen` \| `Firm` \| `City`, `K-58`, rejestr poz. 37). Wtedy podział pliku wychodzi za darmo i wzdłuż tematu, a nie wzdłuż numeru linii ✅ **Zamknięta w `R2-WP26` (2026-09-22): pomiar `struct_guard` nie widzi już tego przekroczenia.** | — |
| 67 | `sim/firms/src/registry.rs` | blok `impl Firms` — **358** linii (ostrzeżenie od 300; 342 po M10d) | Rejestr firm i zakładów: jedna mapa firm, jedna zakładów, jeden licznik kluczy i operacje, które **wszystkie trzy trzymają naraz** — założenie firmy, wstawienie zakładu, lista płac, utarg, zamknięcie zakładu. To nie są dwa tematy w jednym bloku, tylko jeden stan z jednym niezmiennikiem (`Firm.sites` zgodne z kluczami w `sites`), a podział wzdłuż nazw metod rozsypałby go po plikach, w których nikt go nie pilnuje. M10e dołożył tu szesnaście linii: lista płac nie płaci za dni strajku (`K-89`) | Szew tematyczny jest jeden i przebiega **wzdłuż cyklu życia zakładu**: `add_site`, `close_site` i `run_payroll` da się wynieść do `registry/site.rs`, zostawiając rodzicowi mapy, licznik i odczyty (~200 linii). Do zrobienia przez fazę, która jako pierwsza przekroczy tu próg błędu 500 — dziś podział byłby przenoszeniem kodu bez pytania, na które odpowiada **Numer zmieniony z 48 na 67 w `R2-WP26`: dwa wiersze nosiły ten sam numer, więc odesłanie „pozycja 48” nie wskazywało wiersza.** **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 71 ✅ | `tools/balansator/src/gates.rs` | `fn evaluate` — **257** linii (błąd od 250) | **Podzielone od razu, bo podział okazał się mechaniczny.** Prognoza z tej samej pozycji sprzed godziny („dwanaście linii od progu, następna bramka go przekroczy") sprawdziła się w tym samym commicie: dopisanie ogona do G11 przesunęło funkcję na 257. Szew nie był estetyczny, tylko **tematyczny**: `detal` (G1–G6) pyta o ceny i rynek, `rzetelnosc` (G7–G9) o to, czy przebieg w ogóle wolno czytać — czerwień którejkolwiek z tych trzech unieważnia pozostałe bramki — a `firmy` (G10–G11) o populację firm i rynek pracy. `evaluate` zostaje składaniem trzech list i ma dziewięć linii. Zero stanu dzielonego między blokami, więc przeniesienie było przeniesieniem symboli bez zmiany zachowania: ten sam zestaw run-files daje tę samą tabelę | **M7f** **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| — |
| 44 | `game/src/command/check.rs` | `fn precheck` — 198 linii (ostrzeżenie od 150) | Jeden wyczerpujący `match` po `PlayerCommand`. To **nie są** dwa tematy w jednej funkcji, tylko jedna reguła powtórzona szesnaście razy — i jest to jedyny mechanizm, który łapie komendę bez sprawdzenia, bo brak ramienia łamie kompilację. Podział po sztuce odebrałby tę własność | Rozbicie po **dziedzinie** komendy (gospodarka, kadry, miasto, dynastia) na cztery `match`e z jednym dyspozytorem, kiedy komend będzie trzydzieści. Próg błędu 250 padnie tam **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 45 | `game/src/command/exec.rs` | `fn run` — 152 linie (ostrzeżenie od 150) | Sufit ten sam co przy `precheck` i z tego samego powodu. Dłuższe wykonania już wychodzą do funkcji niżej, więc funkcja rośnie **o jedną linię na komendę**, a nie o wykonanie | Razem z `precheck` (pozycja 44) i tym samym podziałem — obie mają wyczerpiwać ten sam enum i mają to robić w tym samym kształcie **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 46 | `game/src/panels/shop.rs` | `fn render` — 172 linie (ostrzeżenie od 150) | Jeden ekran panelu: półka, cena, cel zapasu, asortyment, utracone wizyty, konkurencja. Każda sekcja jest osobnym blokiem i wyjdzie do własnej funkcji bez zmiany kolejności rysowania — dziś podział rozbiłby jeden ekran na pięć plików | Przy pierwszej sekcji, która dołoży do panelu własny stan (np. edytor asortymentu z wyszukiwarką) — wtedy szew jest oczywisty, a nie wymyślony **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 47 | `game/src/panels/mod.rs` | `mod.rs` z własnym kodem — 306 linii (ostrzeżenie od 300) | Opis panelu i jego stan: `PanelId`, `PanelDesc`, `PanelRegistry`, `PanelCtx`, `PanelModel`, `PanelView`, `PanelState`. To jest **kontrakt rozszerzenia** dla M10 i M12 i czyta się go w całości albo wcale; układ doku już z niego wyszedł do `layout.rs` | M10 dokłada trzy panele i wtedy okaże się, czy `PanelModel` ma zostać enumem, czy stać się `Box<dyn Any>`. Ta decyzja i tak przestawi połowę pliku | N8.3 · przegląd 2026-10-22 |
| 48 | `game/src/command/mod.rs` | `mod.rs` z własnym kodem — 355 linii (ostrzeżenie od 300) | Słownik komend gracza: dwadzieścia wariantów `PlayerCommand`, sześć `ViewCommand` i koperta. Walidacja wyszła do `check.rs`, błędy do `error.rs`, wykonanie do `exec.rs` — zostało to, co jest **jedną listą** i czego kolejność jest kontraktem zapisu gry | Nie ma go i nie powinno być: lista rośnie o wariant na komendę i próg 600 osiągnie przy czterdziestu. Wtedy dzieli się **enum**, a to jest zmiana formatu dziennika wejść i osobna decyzja **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 49 | `sim/economy/src/market/restock.rs` | `impl Market` — 327 linii (ostrzeżenie od 300; 285 przed M9e) | Szew zaopatrzenia dostał w M9e cel zapasu sterowany przez gracza (`set_restock_days`, `restock_days`) — dwie metody o tym samym temacie co reszta pliku. Przeniesienie ich gdzie indziej dałoby trzecie miejsce, w którym mowa o polityce zamawiania | M6 albo M10 przy pierwszej zmianie polityki zamówień; wtedy „ile zamawiać" i „kiedy wyłożyć" rozejdą się same | N8.3 · przegląd 2026-10-22 |
| 50 | `tools/magnat/src/session.rs` | `impl App` (przejścia stanów) — 462 linie (ostrzeżenie od 300) | Przejścia stanów gry: generacja, podgląd, wejście do świata, sloty, wybór postaci, edytor reguł. Każde potrzebuje `&mut self` do tych samych pól klienta — ten sam powód co przy pozycji 1 i ta sama odpowiedź. **Podział wzdłuż „powłoka / rozgrywka" już się zaczął**: obsługa doku (komendy paneli, tryb śledzenia, strumień widoku) wyszła w M9e do `dock.rs`, bo przekroczyła próg błędu 500 | M11 dokłada tu ustawienia grafiki i dźwięku. Następny szew idzie tą samą linią: co należy do powłoki, zostaje, co do rozgrywki — wychodzi **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 51 | `game/src/session.rs` | `impl Session` — 321 linii (ostrzeżenie od 300; ~231 przed domknięciem `WP12`) | Domknięcie `WP12` dołożyło tu **dobowy krok gracza** (`doba_gracza`, `life_event`, `settled_outcome`) i to on przekroczył próg. Podział jest dziś niewykonalny mechanicznie: te trzy metody czytają i piszą **prywatne pola** sesji (`legacy_day`, `life_event`, `outcome`), a prywatność w Ruście jest modułowa — `impl Session` w `legacy.rs` ich nie zobaczy. Wyjście jest jedno i ma cenę: wydzielić te pola do własnej struktury (`PlayerClock`?) z własnym `impl`, którą sesja trzyma polem. Warto zrobić to razem z pierwszym następnym stanem gracza liczonym na dobę — dziś byłaby to struktura na trzy pola dla jednego wołania | **M10** (pierwsza faza dokładająca stan gracza liczony dobowo) | N8.3 · przegląd 2026-10-22 |
| 52 | `game/src/panels/dashboard.rs` | `fn render` — 162 linie (ostrzeżenie od 150; ~137 przed `DI-36`) | Sekcja celów scenariusza (`DI-36`) dopisała dwadzieścia linii do ekranu, który był tuż pod progiem. To jest **jeden ekran panelu** — przepływy, alerty, cele, stan posiadania, rozbudowa — i rozcięcie go dałoby cztery funkcje wołane raz, w kolejności, z tego samego miejsca; sufit ten sam co przy poz. 46 (`panels/shop.rs`). Dzieli się wtedy, gdy panel dostanie **drugi tryb**, a nie gdy dostanie kolejną sekcję | — (zostaje świadomie, jak poz. 46) **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 53 ✅ | `tools/magnat/src/app.rs` | plik — 939 linii (ostrzeżenie od 800; było 857 przed M11a) | M11a dołożyła tu **jedną metodę o jednym temacie** (`publikuj_snapshot` — okno warstwy Mikro, zapytanie widoku, wypełnienie snapshotu i publikacja) plus rozbudowaną diagnostykę `--pick`. Podział **teraz** przeniósłby jedną metodę do własnego modułu i zostawił resztę bez zmian — czyli dałby kolejny plik w kliencie i zero czytelności | Pozycja 1 tej tabeli zapowiadała, że szew pokaże się w M11, i właśnie się pokazał: `publikuj_snapshot` jest **pierwszą** metodą klienta, która nie dotyka okna ani wejścia, tylko składa dane dla renderu. Gdy M11b i M11d dołożą do niej pogodę, światła i zakłady, wyjdzie z tego `tools/magnat/src/view.rs` z własnym `impl App` — i wtedy podział ma temat, a nie tylko liczbę ✅ **Zamknięta w `R2-WP26` (2026-09-22): pomiar `struct_guard` nie widzi już tego przekroczenia.**| — |
| 54 | `tools/magnat/src/app.rs` | plik — 1 088 linii (ostrzeżenie od 800, błąd od 1 200; było 939 po M11a) | M11e dołożyła tu **piętnaście linii** (przebieg sceny odniesienia, pomiar selekcji kadru, kursor w środku kadru dla sceny); reszta przyrostu jest z M11b–M11d. Podział teraz byłby podziałem cudzego kodu przy okazji — a szew, który zapowiadał wiersz 53, już się materializuje: `publikuj_snapshot` i `krok_dzwieku` nie dotykają ani okna, ani wejścia | **R2 albo M12.** Wiersz 53 przewidywał `tools/magnat/src/view.rs` i to jest nadal właściwy podział: metody składające dane dla renderu (`publikuj_snapshot`, `zloz_przekroj`, `zloz_szyldy`, `krok_dzwieku`) wychodzą do własnego modułu z własnym `impl App`, a w `app.rs` zostaje pętla okna i wejście. Do progu błędu jest 112 linii, więc następna faza dokładająca tu cokolwiek **musi** ten podział zrobić **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 55 | `engine/render/src/renderer/passes.rs` | blok `impl Renderer` — 399 linii (ostrzeżenie od 300) | **Po podziale, nie przed.** M11e wyprowadziła stąd cały pomiar (`PASS_NAMES`, `FrameStats`, `PassTimer`, `resolve_timer`, `finish_stats`) do `engine/render/src/renderer/stats.rs` — „co klatka rysuje" i „ile to trwało" to dwa tematy. To, co zostało, jest **jednym** tematem: kolejnością passów, a kolejność jest kontraktem. Dalszy podział przeniósłby wycinek sekwencji do drugiego pliku i utrudnił odpowiedź na pytanie, po czym co się rysuje — dokładnie ten sam argument, który wiersz 4 postawił przy `pipelines.rs` | Brak, dopóki nie pojawi się drugi temat. Passy `record_clusters`, `record_shadows`, `record_depth_prepass`, `record_weather` i `record_post` są już osobnymi funkcjami, więc rośnie tu wyłącznie `record_passes`, czyli sama sekwencja **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 56 | `game/src/ambience.rs` | blok `impl Ambience` — 400 linii (ostrzeżenie od 300) | M11e dołożyła jedną metodę o jednym temacie (`dzielnice_do_zgaszenia` — wybór dzielnic sceny `bench_blackout` po liczbie latarni w zasięgu oka). Reszta to M11d. Podział teraz przeniósłby jedną metodę i zostawił resztę bez zmian | **M12** albo pierwsza faza dokładająca tu treść. Szew jest widoczny i przebiega między „stanem atmosfery" (pogoda, zasilanie, rampa wygaszenia) a „listą świateł" — dwa tematy, które dziś dzielą wyłącznie pola bufora latarni | N8.3 · przegląd 2026-10-22 |

| 57 | `engine/render/src/instancing/gpu.rs` | plik — 805 linii (ostrzeżenie od 800; było 790 przed M11e) | M11e dołożyła tu **piętnaście linii**: liczenie wywołań rysowania tam, gdzie powstają, i histogram instancji per poziom detalu. Plik jest jednym tematem — strona GPU instancingu: bufory, potoki, wgrywanie i rysowanie — a podział przeniósłby wycinek tworzenia zasobów do drugiego pliku. Ten sam argument co przy wierszu 4 (`pipelines.rs`) | Brak, dopóki nie pojawi się drugi temat. Najbliższym kandydatem jest atlas sylwetek (`impostor_texture`, `impostors_to_gpu`, dwa potoki impostorów), który z buforem instancji dzieli wyłącznie grupę wiązań — wyszedłby przy pierwszej fazie dokładającej do impostorów treść **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |

| 58 ✅ | `sim/macro/src/dryrun.rs` → `sim/macro/src/verify.rs` | plik — 818 linii po M10f (ostrzeżenie od 800; 761 przed) | **Podzielone od razu, bo podział okazał się mechaniczny i tematyczny.** M10f dodało tu bramkę 10 (Gini dochodu) i rozdzieliło bramkę 7 (`D9`), co przesunęło plik nad próg. Szew był widoczny od początku: `dryrun` **przeprowadza** historię (zasiew, bieg, kronika, rozwinięcie, naprawa), a bramki **oceniają** stan, który z niej wyszedł, i nie wiedzą nic o tym, jak powstał. Widać to po wejściach: żadna funkcja bramkowa nie bierze `&mut`. Przeniesienie symboli bez zmiany zachowania — 522 + 322 linie, ten sam raport z tego samego ziarna | **M10f** — zrobione ✅ **Zamknięta w `R2-WP26` (2026-09-22): pomiar `struct_guard` nie widzi już tego przekroczenia.**| — |
| 59 | `tools/balansator/src/gates.rs` | plik — **1 109** linii po R2f (989 po M10f (ostrzeżenie od 800, błąd od 1200; 869 przed M10f) | Bramka `G12` (makro vs mezo) dopisała tu 120 linii razem z testami, strażnikiem doby zerowej i rozbiciem werdyktu na serie. Plik jest **jedną listą bramek** i czyta się go tak, jak czyta się tabelę §7.4 dokumentu M5 — od G1 do G12. Podział po rodzinach już raz został zrobiony (pozycja 41: `detal`, `rzetelnosc`, `firmy`) i to był właściwy szew; drugi rozbiłby listę na pliki, między którymi trzeba by skakać, żeby sprawdzić, czy numer bramki jest wolny | **Przy pierwszej bramce, która dokłada własny stan** (np. próg czytany z danych zamiast stałej). Wtedy progi i logika rozejdą się same: `gates/progi.rs` obok `gates/mod.rs`. Do progu błędu zostaje 211 linii **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 60 | `sim/media/src/system.rs` | plik — 802 linie po M10f (ostrzeżenie od 800; 799 przed), `fn` publikacji — 152 linie (ostrzeżenie od 150; 149 przed) | M10f dodało tu **trzy linie**: licznik tekstów uderzających w markę (`FF-25`). Plik przekroczył próg o dwie linie i funkcja o dwie — podział uruchomiony tą zmianą byłby podziałem cudzego kodu przy okazji trzech linii. Funkcja jest przy tym **jednym przebiegiem redakcji**: wybór zdarzenia, teza, zasięg, ekspozycja czytelników, uderzenie w markę bohatera, publikacja — i każdy krok czyta wynik poprzedniego | **M10g albo R2.** Szew jest widoczny i przebiega między „redakcja wybiera, o czym pisać" a „tekst dociera do czytelników": pierwsze jest decyzją tytułu, drugie rozkładem po dzielnicach. Dziś dzielą wyłącznie zmienną `story` **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 61 | `game/src/chronicle.rs` | blok `impl Chronicle` — 326 linii (ostrzeżenie od 300; ~200 przed M10f) | M10f dodało dwa zbieracze (`zbierz_wladze`, `ingest_dry_run`) i to one przekroczyły próg. Podział jest dziś **niewykonalny mechanicznie** z tego samego powodu co przy pozycji 51: każdy zbieracz pisze przez prywatne `push` i `nowy_id` i przesuwa własny prywatny kursor, a prywatność w Ruście jest modułowa — `impl Chronicle` w drugim pliku ich nie zobaczy. Wyjście ma cenę: kursory do własnej struktury (`Kursory`) z polami `pub(crate)`, a zbieracze jako funkcje wolne biorące `&mut Chronicle` | **M10g albo R2.** M10g dokłada tu dokładnie jeden zbieracz (negocjacje gracza w kronice) i wtedy szew będzie miał dwóch beneficjentów zamiast jednego. Do progu błędu 500 zostaje 174 linie **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |

| 62 | `sim/agents/src/migration.rs` | plik — **1 339** linii po R2b (ostrzeżenie od 800, **błąd od 1 200**; 1 274 przed) | R2b dołożyła tu podział majątku przy wyprowadzce (`Czesci`, `wydziel_udzial`, `R2-WP8`) i naprawę dopisywania członka poza `debug_assert!` (poz. 77 wykazu `R2`). Plik jest długi, bo trzyma trzy tematy naraz: pulę wakatów i pustostanów (`Vacancies`, ~180 linii), spawn mieszkańca i gospodarstwa (~260) oraz sam regulator populacji — napływ, odpływ, usamodzielnienie (~500). Szew jest naturalny i przebiega po tych trzech: `migration/vacancies.rs`, `migration/spawn.rs`, `migration/flow.rs`. **Nie dzielę teraz**, bo `spawn_household_aged` i `zaloz_gospodarstwo` są w tym samym commicie zmieniane merytorycznie, a przeniesienie ich w tej samej zmianie zamieniłoby recenzowalny diff w „plik przeniesiony” | Pierwsza faza, która dotknie regulatora populacji bez zmiany spawnu — albo R3, jeśli wcześniej nic tu nie wejdzie **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 63 | `sim/economy/src/market/api.rs` | blok `impl Market` — **541** linii po R2b (ostrzeżenie od 300, **błąd od 500**; 499 przed, patrz pozycja 42) | Pozycja 42 zapowiadała to wprost: „kto dotknie tego pliku jako następny, przekroczy próg błędu”. R2b dotknęła — dwa odczyty i dwa zapisy dla kont kanałów mobilnych (`K-72`) oraz licznik wypłat miesiąca (`R2-WP30`). Szew jest ten sam, co w pozycji 42 i nie zmienił się od R1: „odczyt dla prezentacji i pomiaru” → `api/readout.rs`, zapisy zostają. **Nie dzielę w tym commicie**, bo przeniesienie czterdziestu metod w zmianie dotykającej sześciu skrzyń i czterdziestu plików czyni ją nie do przeczytania — a podział jest mechaniczny, bez zmiany zachowania, więc należy do **własnego commitu**. To jest ta sama odpowiedź, którą M6d dało pozycji 38, i z tego samego powodu | **Osobny commit zaraz po R2b**, przed R2c — nie faza, bo podział niczego nie potrzebuje poza sobą **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 64 | `sim/agents/src/demography/mod.rs` | `mod.rs` z własnym kodem — **634** linie po R2b (ostrzeżenie od 300, **błąd od 600**; ~597 przed) | R2b rozszerzyła tu `InheritanceHook` o `&mut World`, `estate_charge` i `set_day` (`K-95`) — trzydzieści kilka linii kontraktu i jego uzasadnienia. Warunek podziału jest nazwany w pozycji 21 i **nadal nie zaszedł**: narodziny i zgon wciąż same piszą relacje rodzinne, więc piątka relacyjna nie ma dokąd pójść (`D-16`). Zmieniło się natomiast co innego i to jest treścią tego wiersza: `InheritanceHook` **ma już drugą implementację** (`sim/economy::inherit`), więc trait z jego uzasadnieniem przestał być zaślepką i ma własny powód istnienia — czyli własny plik `demography/inheritance.rs`, którego przedtem nie było czym uzasadnić | Pierwsza faza dotykająca haka dziedziczenia; szew: `InheritanceHook` + `NoInheritance` (~60 linii) do `demography/inheritance.rs`, co schodzi `mod.rs` poniżej progu błędu **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 65 | `sim/economy/src/data.rs` | `fn EconomyData::load` — **270** linii po R2b (błąd od 250; 269 przed, patrz pozycja 38) | Wyzwalacz z pozycji 38 **nie zadziałał po raz czwarty**, a funkcja urosła o jedną linię: `R2-WP11` dopisał do `envelopes.ron` sekcję `equivalence`, czyli **nowe pole w istniejącym bloku**, a nie nowy czytnik. `BudgetParams` buduje się dokładnie tam, gdzie się budował, o jeden wiersz dłużej. Odpowiedź ta sama, którą dały M6d i M7d: podział pliku, którego zmiana dotyczy w jednej linii, w commicie zamykającym podfazę, jest przejazdem przy okazji **Sprostowanie po R2c: wyzwalacz nie zadziałał po raz piąty, a funkcja nie urosła ani o linię (270).** `R2-WP16` usunęła `NeedKind::Status`, więc `weights.ron` i `choice.ron` mają o **wiersz mniej**, a `ECONOMY_SCHEMA_VERSION` poszło z 4 na 5 — czyli ubyło danych, a nie przybyło czytelników. `load` czyta dokładnie te same siedem plików tym samym rytuałem. | Bez zmian wobec pozycji 38: pierwsza faza, która dopisze do `data/economy/` **nowy plik albo nowy czytnik**, a nie pole do istniejącego **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.**| N8.3 · przegląd 2026-10-22 |
| 66 | `sim/supply/src/plant/produce.rs` | plik — **843** linie po R2c (ostrzeżenie od 800, błąd od 1 200; 762 przed) | `R2-WP14` dołożyła tu wykonawcę szóstego szczebla kaskady: pytanie o zamiennik przy sprawdzaniu wsadu i drugi przebieg pobierania, który dobiera brakującą resztę z `RecipeInput.substitutes` w proporcji z danych. To jest **ten sam temat**, co reszta pliku — życie linii produkcyjnej minuta po minucie: start, przezbrojenie, pobranie wsadu, zamknięcie szarży — a nie drugi temat obok niego. Dzieli się pliki, w których są dwa tematy, a nie pliki, które są długie. **Szew, gdyby kiedyś był potrzebny, jest mechaniczny i nazwany:** `pobierz_wsad` z `wydobadz` i pomocnikami podmiany to gotowy `charge.rs` (~210 linii), a reszta zostaje przy pętli minuty. | **Pierwsza faza, która dołoży linii produkcyjnej trzeci temat** — najbliższy kandydat to M12c (churn encji w trybie 50×) albo faza wprowadzająca nadgodziny i drugą zmianę na tej samej linii (sufit nazwany w `Site::labor_pct`) | N8.3 · przegląd 2026-10-22 |
| 68 | `sim/world/src/city/mod.rs` | metryka `mod.rs` — **603** linie kodu poza deklaracjami (ostrzeżenie od 300, **błąd od 600**; 600 przed R2f) | `R2-WP25` dołożyło ramię `WorldSize::Km2` do `target_pop` razem z komentarzem mówiącym, czym ten rozmiar jest i czego nie robi. Trzy linie — i metryka przesunęła się z ostrzeżenia na błąd, bo stała **dokładnie na progu**. Plik jest orkiestratorem generacji miasta: jedenaście etapów po kolei, z których każdy ma własny moduł | Rozcięcie tego pliku znaczy rozcięcie kolejności etapów, a `R3-WP1` i tak przestawia Etap 7 — wtedy podział ma temat, a nie tylko liczbę **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.** | N8.3 · przegląd 2026-10-22 |
| 69 | `sim/economy/src/books.rs` | plik — **908** linii (ostrzeżenie od 800; **890 przed R2f**, czyli nad progiem już wtedy i bez wiersza w tym rejestrze) | Księga główna: konta, przelewy, dziennik transakcji i niezmienniki pieniądza. Jeden temat i jedna odpowiedzialność — rozcięcie go znaczy rozcięcie niezmiennika `P1`, który sprawdza sumę **wszystkich** kanałów naraz. `R2-WP39` dołożył tu osiemnaście linii: parametr `reason` w `inject_external_capital` i akapit mówiący, dlaczego wołający musi go podać | **Adres wygasł — pozycja przechodzi do `R3` (`R3-etap-7-i-dlug-strukturalny.md`); pomiar `R2-WP26`, 2026-09-22.** Wpisana dlatego, że plik stał nad progiem **bez wiersza**, a reguła przeglądu strukturalnego każe odpowiedzieć, a nie przemilczeć | N8.3 · przegląd 2026-10-22 |
| 72 | `sim/traffic/src/oracle.rs` | `mod.rs` (pisownia `oracle.rs` obok `oracle/`) — **864** linie własnego kodu (błąd od 600) | Metryka nowa od `N1.7`: rodzic katalogu podmodułów do E1 nie był mierzony jak `mod.rs`, więc przekroczenie istnieje od dawna, a nie od tego commitu. Czy plik niesie dwa tematy, czy jeden długi — ocena należy do przeglądu rejestru, nie do naprawy bramki | Przeniesienie kodu do podmodułów w `oracle/` albo świadome zostawienie z `ponytail:` | N8.3 · przegląd 2026-10-22 |
| 73 | `sim/economy/src/market.rs` | `mod.rs` (pisownia `market.rs` obok `market/`) — **806** linii własnego kodu (błąd od 600) | Jak poz. 72 | Jak poz. 72, w `market/` | N8.3 · przegląd 2026-10-22 |
...
| 81 | `engine/render/src/renderer.rs` | `mod.rs` (pisownia `renderer.rs` obok `renderer/`) — **614** linii własnego kodu (błąd od 600) | Jak poz. 72 | Jak poz. 72, w `renderer/` | N8.3 · przegląd 2026-10-22 |
...
| # | Zmiana | Dlaczego |
|---|---|---|
| D-32 ★ | **Job `struct-guard` w CI był czerwony od M9a i od M11c naraz, a nikt tego nie zauważył.** Dwie niezależne przyczyny: `--self-test` sprawdzał, czy mierzony jest `tools/headless/src/retail.rs` — pliku nie ma od `K-68`, bo most detaliczny przeniósł się do `game/src/world/retail.rs`; `--all` łapał dwa przekroczenia, których M11c nie zamroziło (`lsystem.rs` impl 605 → 609, `city/mod.rs` fn 368 → 370). Obie naprawione w M10c: fikstura wskazuje `tools/headless/src/population.rs`, a rejestr zamraża nowe wartości z komentarzem nazywającym datę i przyczynę dryfu | Bramka, która świeci na czerwono niezależnie od zmiany, przestaje cokolwiek mówić — i to jest dokładnie ryzyko `R-5` opisane w nagłówku rejestru („bramka, która zawsze świeci na czerwono, zostanie wyłączona”). Przez trzy podfazy nikt nie wiedział, czy jego zmiana coś przekroczyła, bo odpowiedź brzmiała „tak” zawsze. Odmrożenie **nie jest zgodą na wzrost**: pozycje 24 i 33 rejestru i ich adresy zostają bez zmian, a klucz dalej niesie wartość, więc następna dopisana linia zapala bramkę z powrotem. Wniosek na przyszłość jest ten sam, który M10b zapisało o martwej ładowarce `brand.ron` (`F-22`): **mechanizm, którego wynik nikogo nie zatrzymuje, psuje się cicho i wychodzi dopiero w cudzej zmianie** |

---

## Zmiany wpisane po R2f

Zgodnie z `K-18`. R2f dotknęło tego dokumentu w trzech miejscach naraz, bo trzy jego
decyzje otwarte miały tu swój adres: `D-R7` (testy poza CI), `D-R8` (linia bazowa
benchmarków) i sam rejestr długu strukturalnego.

| # | Zmiana | Dlaczego |
|---|---|---|
| T-2 ★ | **Rejestr długu ma egzekutora i pierwszy przebieg pokazał, że reguła „pozycja ma adresata" nie działała.** `python scripts/struct_guard.py --all` czyta od `R2-WP26` tabelę rejestru i sprawdza dwie rzeczy: czy dokument wskazanej fazy istnieje i czy faza nie jest odhaczona w `00-postep.md`. Wynik pierwszego uruchomienia: **55 pozycji z 68 bez żywego adresata** — dziewięć zamkniętych i nieodnotowanych, czterdzieści dziewięć z adresem, który minął. Dwa wiersze nosiły ten sam numer (41 i 48), a tabela była **rozcięta poziomą kreską**, więc jej druga połowa od pozycji 44 nie renderowała się jako tabela | Plan R2 przewidywał **cztery** takie pozycje. Różnica rzędu wielkości ma jeden powód: rejestr powstał po M5 przy założeniu, że M6–M12 otworzą te pliki z własnych powodów, a M6–M11 otworzyły część i zamknęły się. Reguła bez egzekutora działa dokładnie tak długo, jak długo ktoś o niej pamięta |
| T-3 ★ | **Czterdzieści dziewięć pozycji przechodzi do `R3-etap-7-i-dlug-strukturalny.md`**, a dziewięć dostaje znacznik `✅`. Dokument R3 powstał w R2f i **nie z pomysłu, tylko z konieczności**: `D-N20` przeniosło tam `R2-WP18` (przeprojektowanie Etapu 7) już w R2d, a dokumentu nie było — czyli adresat, którego bramka `struct_guard` nie umiałaby odróżnić od literówki | Rozsypanie tych pozycji po pięciu podfazach M12 byłoby siódmą przeprowadzką tej samej pozycji. M12 to pamięć, zapis, tryb 50×, modding i lokalizacja — żadna z nich nie jest fazą od dzielenia cudzych plików |
| T-4 | **`D-R7` zamknięte pomiarem: 27 testów `sim/world/tests/` wychodzi z `#[ignore]`.** Nie przez skrócenie testów, tylko przez skrócenie świata — `WorldSize::Km2` (2 048 m) ma tę samą strukturę co 4 km i generuje się w ułamku czasu. Stan po R2f w tabeli niżej | `D-R7` miał adres (`R2-WP25`) i to jest jedyna z trzech decyzji R1, która go miała. Hash parametrów generacji pisze `meters()` jako `u32`, a nie dyskryminantę, więc macierz 32 ziaren × 5 regionów i macierz 32 hashy miasta wyszły **identyczne bit w bit** — sprawdzone, nie założone |
| T-5 | **`D-R8` zamknięte: brak wpisu w linii bazowej jest od R2f błędem, nie komunikatem `NOWY`.** `bench_guard` dostał przy okazji `--self-test` — do R2f nie miał testu samego siebie i dlatego przez sześć faz nie zauważył, że dla trzynastu z czterdziestu jeden pozycji nie mierzy niczego | Pytanie „kiedy odnawia się linię bazową", którego `D-R8` nie rozstrzygnął, ma odpowiedź w `D-N21` (R2f §5.19): **przy zamknięciu każdej fazy**, jednym commitem bez innych zmian. Wariant z `D-8` („przy świadomej zmianie wydajności") jest czystszy teoretycznie i przegrał empirycznie |

### `D-R7` — stan po R2f

Kolumna „automatycznie" znaczy: chodzi przy zwykłym `cargo test -p magnat-world`, bez
`--include-ignored`. Kolumna „nocą" wymienia testy, które **zostają** `#[ignore]` — każdy
z powodem i z nazwą joba CI, a nie z formułką „generacja świata".

| Plik | Automatycznie | Nocą (job `determinism`) | Dlaczego te zostają |
|---|---|---|---|
| `tests/city.rs` | **9 z 12** | `bramy_wymagane_istnieja_i_leza_na_wlasciwym_terenie`, `kopalnia_stoi_na_zlozu`, `etap_3_miesci_sie_w_budzecie_czasu` | Na 2 km brama portowa dla ziarna 8 nie trafia do sieci, `Lowland` nie dostaje ani jednego zakładu wydobywczego (kryterium spełnione tożsamościowo), a budżet czasu jest liczbą **dla miasta 40-tysięcznego w `release`** |
| `tests/city_m2c.rs` | **5 z 9** | dzielnice, bocznice klastrów, parcelacja, determinizm dwóch przebiegów | Potrzebują świata, w którym jest co dzielić — na 2 km nie ma próbki |
| `tests/city_m2d.rs` | **9 z 16** | wycena (`pass_1`/`pass_2`), pokrycie katalogu gramatyk, powierzchnia lokali, niweleta, różnorodność zabudowy, park i kopalnia, `jezdnia_lezy_na_niwelecie` | Mierzą **rozkłady**, a rozkład na ćwiartce powierzchni mierzy wariancję próbki, nie generator |
| `tests/population.rs` | **4 z 7** | dwa dopasowania statystyczne oraz `doba_przez_systemy_ecs_planuje_dowozi_i_zaspokaja` | 2 km daje 5 102 mieszkańców, czyli za małą próbkę; trzeci wywraca się na `debug_assert` kolejki zdarzeń (**poz. 73 wykazu R2**) i to jest jego prawdziwy powód, a nie rozmiar mapy |
| `tests/determinism.rs` | 3 z 5 | macierz hashy terenu, generator macierzy | Bez zmian: 160 generacji to bieg nocny z definicji |
| `tests/budgets.rs`, `mesh_budget.rs`, `lod.rs`, `consistency.rs`, `lsystem_split.rs` | bez zmian | bez zmian | Budżety 16 km, macierz hashy miasta i spójność LOD **mierzą skalę**, a nie strukturę — 2 km nie ma tam czego zastąpić |

Czas pełnego `cargo test -p magnat-world` po zmianie: **4 min 57 s** w profilu `debug`
z kompilacją (kryterium `R2-WP25`: poniżej pięciu minut). Trzy awarie, które `D-R7`
diagnozował w `D-19` i `D-22`, **zostają nierozstrzygnięte** — testy dalej padają na 4 km
i dalej nie biegną automatycznie; R2f zmienił to, że mają wiersz z powodem i job, a nie
formułkę „generacja świata".

### `D-R8` — stan po R2f

| | |
|---|---|
| Pozycji w `benches/baseline.json` przed R2f | 71 |
| Benchmarków zgłaszających `NOWY` (bez wpisu) | 13 z 41 mierzonych w CI |
| Co się zmieniło | brak wpisu → **kod wyjścia 1**, z nazwą benchmarku; `--self-test` z sześcioma przypadkami; wyjątek wyłącznie dla benchmarku dopisanego do linii bazowej **w tym samym commicie** |
| Kiedy odnawiać | przy zamknięciu każdej fazy, jednym commitem bez innych zmian (`D-N21`) |
