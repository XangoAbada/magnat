...
|---|---|---|---|---|
| R2-WP21 | Generator dróg: rozcięcie `lsystem.rs` | — | M | `[x]` (macierz 32 hashy miasta identyczna bit w bit) |
| R2-WP22 | Martwe warianty i nieużywane pola | — | M | `[x]` (dziewięć tematów; poz. 63 zamknięta sprostowaniem, poz. 55 weryfikacją) |
| R2-WP23 | Dokumentacja wejściowa i zakresy strumieni | — | S | `[x]` (`scripts/plan_guard.py`; `StreamId` i liczebnik — weryfikacja) |
| R2-WP28 | Liczba i tekst dla gracza bez niespodzianek | — | S | `[x]` (trzy pozycje; przy okazji kwota w groszach na ekranie) |
| R2-WP31 | Wpłata poza rejestrem to nie praktyka monopolistyczna | — | M | `[x]` (`K-73`; zmierzone 42 sprawy przy zerze zakładów ponad progiem) |
| R2-WP27 | Jeden język w kodzie: identyfikatory i komunikaty | `D-N19` | S | `[x]` (`scripts/lang_guard.py`; siedem identyfikatorów z diakrytykiem) |
| R2-WP20 | Podział `DecisionReason` | wszystkie pozostałe pakiety R2 | L | `[x]` (`K-58`; kryterium rozmiaru skorygowane pomiarem, `E-8`) |
...
| # | Zmiana | Dlaczego |
|---|---|---|
| `E-1` | **`README.md` nie mówił „M1 zamknięte", tylko „M8 zamknięte, M9a zamknięte".** Rozjazd był realny (repozytorium jest po R2d), ale mniejszy niż zapisany w §R2-WP23 | Nagłówek stanu był ruszany w M9a i nikt tego nie odnotował. Sama diagnoza „nie był ruszany od pierwszej fazy" jest nieprawdziwa; wniosek — że opis rozjeżdża się cicho, bo nie ma z czym się nie zgadzać — zostaje |
| `E-2` | **Wariantów nagłówka tabel korekt było siedem w 22 nagłówkach, nie cztery.** Doszły „Korekty planu wpisane w trakcie MX", „Korekty planu wpisane w MX" i wariant bez nazwy fazy | Przegląd liczył wystąpienia w treści, a nie nagłówki, więc trzy warianty z jednym–dwoma wystąpieniami wypadły. Ujednolicone wszystkie 21; `## 4a. Korekty planu wprowadzone w trakcie implementacji` w `M0` **zostaje**, bo jest numerowaną sekcją z żywymi odsyłaczami `M0 §4a`, a nie tabelą doklejoną na końcu |
| `E-3` | **Job balansatora nazywa się „G1–G12", nie „G1–G11".** Bramek jest dwanaście: `G12` (odchylenie makro od mezo) powstała w M10 | Ta sama klasa błędu co sama pozycja: nazwa opisywała stan sprzed dwóch faz |
| `E-4` ★ | **`Catalog::load` sprawdzał liczbę form liczebnika już przed R2e** (`LocError::PluralArity`, od `DE-8` w M9b). Zdanie „dziś wczytuje się i gubi formę ułamkową po cichu" było nieprawdziwe | Mechanizm był, brakowało **testu** i poprawki reguły w `CLAUDE.md`. R2e dokłada jedno i drugie; pakiet nie dokłada mechanizmu, bo nie ma czego |
| `E-5` ★ | **Blok `StreamId` 240–259 był wykonany w całości i bez duplikatów.** Pakiet sprowadził się do weryfikacji i do dopisania testu | Kompilator łapie duplikat dyskryminanty sam (`E0081`), więc test na to byłby powieleniem `rustc`. Test pilnuje za to tego, czego kompilator nie widzi: **wariantu bez jawnego numeru**, który bierze go z pozycji w pliku, oraz numeru spoza bloku fazy |
| `E-6` ★ | **Kryterium `R2-WP27` („lista symboli publicznych bez znaku spoza ASCII") jest spełnione tożsamościowo.** W repozytorium nie ma ani jednego identyfikatora publicznego spoza ASCII i nigdy nie było | Wykrywacz słownikowy napisany zamiast niego **przepuścił podłożone `pub fn pomnoz_ulamek`**: nazwa nie ma cechy odróżniającej polski od angielskiego, której da się poszukać wyrażeniem regularnym. Bramką została reguła, którą da się sprawdzić — **każdy identyfikator w ASCII** — i ona **padła na siedmiu nazwach z diakrytykiem**, wszystkich w nazwach testów. Skan morfemowy zostaje pod `--list` jako pomoc do przeglądu, z nazwanym sufitem |
| `E-7` ★ | **Trzecia kategoria z `D-N19` opisuje polski, nie angielski.** Komunikatów deweloperskich po polsku jest **550** (148 poza `#[cfg(test)]`) | Reguła „komunikaty po angielsku" stworzyłaby dokładnie ten rozjazd, który `R2-WP27` zamyka. Decyzja właściciela produktu z 2026-09-21: komunikat czyta ten sam człowiek i w tym samym zdaniu co komentarz nad nim, więc idzie w języku tego komentarza |
| `E-8` ★ | **Kryterium `R2-WP20` („trzy funkcje poniżej 300 linii każda") było nieosiągalne z arytmetyki.** 1290 linii ramion na trzech aktorów to średnio 430; wyszło 376 · 634 · 287 | Dalszy podział **wewnątrz aktora** nie składa się w nic mniejszego — dyspozytor musiałby wymienić te same 54 warianty, żeby wiedzieć, do którego pliku je posłać. Aktor jest jedyną osią, wzdłuż której ten `match` da się rozciąć raz. Zysk jest za to trwały i to on był celem: funkcja przestała rosnąć z liczbą **faz** i rośnie z liczbą decyzji **swojego** aktora. Trzy funkcje mają wiersz w rejestrze długu z nazwanym sufitem |
| `E-9` | **`Proposal` zostaje w `lsystem/mod.rs`, a nie przenosi się do `rules.rs`**, jak zapowiadała tabela §R2-WP21 | `Proposal` jest ładunkiem **kolejki priorytetowej**, a kolejka jest w `mod.rs`. Przeniesienie kosztowałoby `pub(super)` na dziewięciu polach i nie dodałoby ani jednej granicy tematycznej |
| `E-10` ★ | **Pozycja 35 rejestru długu (`Builder::grow`, 261 linii) zostaje otwarta** — zamknęły się 33 i 34 | `grow` jest **jednym algorytmem**: drabiną ograniczeń, w której kolejność kroków jest kontraktem generacji miasta. Podział na dwie funkcje przeniósłby połowę drabiny piętro niżej bez granicy tematycznej, a zamiana dwóch kroków miejscami przy okazji przenosin przestawiłaby każde miasto z każdego ziarna |
| `E-11` ★ | **Pozycja 63 wykazu (`UtilityKind` vs `UtilityService`) zamyka się sprostowaniem, nie naprawą.** To są **dwa różne pojęcia** o mylnie podobnych nazwach: wymiar oceny w funkcji użyteczności i medium komunalne | `CB-1` z M8 wykonano w całości; przegląd odczytał dwa żywe identyfikatory jako połowiczne wykonanie. Oba mają czytelników (33 i 170 wystąpień), żaden nie jest martwy. Różnica jest od tej chwili nazwana w `vocab.rs` po obu stronach — i to jest zamknięcie, które dokument sam przewidywał |
| `E-12` ★ | **`TripPurpose` przenosi się do `engine/core` i `TripRequest` niesie cel podróży** — zakres większy niż „`Escort` dostaje konstruktora" | Pomiar pokazał, że martwy jest nie jeden wariant, tylko **sześć z siedmiu**: `TrafficOracle::start_trip` wpisywał każdej podróży `Work`, więc z siedmiu mnożników w `data/roads/mode_choice.ron` żył jeden. To jedna przyczyna („podróż nie niosła swojego celu"), więc jedna naprawa — a `Escort` bez niej nie miał jak powstać |
| `E-13` | **Pozycja 54 (`radius_m`) dostała nośnik danych, a nie tylko drugi promień.** `CompetitorEntry` niesie `near: [NearRadius; 2]`, a `Shop` — `asked_radii` zgłaszane przez politykę | `observe_competitors` nie widzi polityki zakładu, więc promieni musi się o nich **dowiedzieć**: zapisuje je `run_policies` poprzedniej doby. Opóźnienie o dobę jest tym samym, które ma cały obraz konkurencji (1–7 dób), i nie wnosi nowej klasy błędu |
| `E-14` | **Pozycja 55 zamyka się weryfikacją, a przy okazji wychodzi pozycja 81:** `PolicyDomain::Logistics` nie ma **ani jednej akcji**, podczas gdy `Hr` ma dwie, a `Production` jedną | Odmowa walidatora (`DomainNotAvailable`) działa i ma test, więc cichej polityki nie ma. Wariant bez akcji nie znika tutaj, bo `Policy.domain` jedzie w zapisie gry — należy do fazy, która transport zleceń zbuduje, albo do M12b razem z migracją |
| `E-16` ★ | **Bramka `struct_guard` świeciła na czerwono od R2a i nikt tego nie zauważył.** Cztery przekroczenia progu błędu (`migration.rs`, `demography/mod.rs`, `market/api.rs`, `economy/data.rs`) miały wiersze w rejestrze długu (62–65) **i nie miały wpisu w `REJESTR` skryptu**, więc job CI był czerwony trzy podfazy z rzędu | Dokładnie ten mechanizm, przed którym ostrzega komentarz nad `REJESTR`: bramka, która zawsze świeci na czerwono, przestaje cokolwiek znaczyć i zostaje wyłączona. Zamrożone z wartościami zastanymi — zamrożenie **nie jest zgodą na wzrost**, bo klucz niesie liczbę. Przy okazji `--self-test` złapał martwy wpis (pozycja 38, wartość sprzed R2c), który został usunięty |
| `E-15` | **`Household` schudł ze 120 B do 112 B**, a nie „zmniejszył się o dwa pola i zostawił rezerwę" | `shopper_rotation` (1 B) i `vehicle_slots` (8 B) wypadły razem z trzema bajtami wyrównania. Osiem bajtów na gospodarstwo to 3,2 MB na metropolii. Karta mieszkańca czyta pojazdy przez `VehicleOwner.owner` — pole miało czytelnika i **nie miało pisarza**, więc lista pojazdów była pusta od M4 |
