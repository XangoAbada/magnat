...
Zgodnie z `K-18`. Pełna tabela `I-n` z uzasadnieniami jest w `M11d-swiatlo-pogoda-dzwiek.md`;
tutaj wyłącznie to, co dotyczy **całej fazy**. Gwiazdka = zmiana zakresu albo kryterium.

| # | Zmiana | Dlaczego |
|---|---|---|
| V-1 ★ | **Decyzja 9.6 zamknięta wg propozycji domyślnej: `engine/audio` stoi na `kira`.** Mikser, cztery magistrale, limitery i przejścia są jej; nasze są emitery przestrzenne, okluzja, klastrowanie głosów, `MusicDirector` i synteza źródeł | Prototyp z WP8 potwierdził założenie: wszystko, czego potrzebuje §5.9 poza wiedzą o **tej** grze, `kira` już ma. Reguły niosące informację dla gracza siedzą po naszej stronie jako funkcja czysta (`plan`), więc **komplet testów z §7.4 biegnie bez karty dźwiękowej** — to jest zysk, którego żadna z dwóch bibliotek sama by nie dała |
| V-2 ★ | **`data/audio/` nie ma nagrań: katalog opisuje syntezę** (decyzja właściciela produktu z 2026-09-19, kontrakt w `K-77`) | Ten sam wzorzec, którym `data/models/` napełnia się przez `mvoxc gen` (`Y-6`). Sufit jest jawny — to brzmi jak maszyna, nie jak nagranie — a ścieżka wyjścia nie rusza ani jednej linii kodu syntezatora: katalog dostaje pole ze ścieżką pliku, a `kira` umie go wczytać sam |
| V-3 ★ | **Sekcje `weather`, `power` i `lights` snapshotu są wypełnione**, a `crowd` zostaje jedyną wyzerowaną — warstwy L3 nie ma od `W-2` | `H-1` zapowiadało, że wypełni je ta podfaza, i tak się stało. Wypełniacz atmosfery mieszka w `magnat_game::ambience`, obok `view.rs`, a nie w nim: to inny temat i inny zestaw źródeł |
| V-4 ★ | **`no_sim_deps_in_render` obejmuje od tej chwili `magnat-audio`** — bramka `dep_isolation` w CI wymienia trzy crate'y, nie dwa | §6.3 pkt 1 wymieniał audio obok renderu od początku, a pętla w `ci.yml` nie. Nowy crate bez wpisu byłby jedynym, którego nikt nie pilnuje |
| V-5 ★ | **Ryzyko `R8` (skala `activity` i `emission`) zamknięte inaczej, niż zakładała decyzja 9.4**: `emission` liczy się z **tempa** receptury, a widoczność pióropusza z `plume_kind` i `activity` | M6 dał skalę bezwzględną i to się nie zmienia, ale źródłem nie może być `pm_g_last_minute`: pole napełnia się w minucie zakończenia szarży i przez resztę czasu jest zerem (`I-6`). Przy okazji wyszło, że para z chłodni ma zerowy pył — więc sam pył nie mógłby o widoczności decydować (`I-7`) |
| V-6 ★ | **Usterka sprzed M11d: wypełniacz zakładów nie trafiał w rejestr produkcji** (`K-46` — dwie numeracje `SiteId`), więc od M11c każdy zakład miał aktywność 128 i zapas 255 | To są wartości domyślne `fill_sites` i wyglądają jak prawdziwe. Kryterium `interior_reflects_stock` z §7.3 przechodziło na stałej, a nie na stanie magazynu — wraca do M11e razem ze scenami odniesienia |
| V-7 | **`PASS_NAMES` ma siedem pozycji**; `weather` jest siódmą, a pomiar `pick_id` wejdzie jako ósma | Indeksy znaczników czasu są pozycyjne, więc dopisuje się je na końcu — inaczej porównanie z baseline'em mierzy przenumerowanie, nie zmianę kodu |
