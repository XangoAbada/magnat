...
Poprawki wpisane przez podfazę **M7f** samą w sobie (`K-18`) — to, co projekt tej podfazy
obiecywał inaczej, niż wyszło. Korekty dokumentu **fazy** są w tabeli „Zmiany wpisane po M7f"
w `M7-firmy-ai-i-rynek-pracy.md`. Gwiazdka = zmiana zakresu albo kryterium.

| # | Zmiana | Dlaczego |
|---|---|---|
| `BE-1`★ | **`rank_variants` nie mieszka w `firms::ai::strategic`, tylko w `sim/macro`**, a tier strategiczny nie mieszka w `sim/economy::ai_run` razem z dwoma pozostałymi — ma własny system `macro.WhatIf` w `sim/macro`. Wynik przechodzi przez `firms::StrategicOutlooks`, zasób świata zdefiniowany w `sim/firms` | Adres z §5.10 jest niewykonalny: `sim/macro` woła jądro (`K-50`), więc `macro → economy`, a `economy → firms → policy`. `rank_variants` w `sim/firms` znaczyłoby `firms → macro → economy → firms`, czyli cykl, którego Cargo nie zbuduje. Rozstrzygnięcie w `K-54`. **Wychodzi z tego gwarancja mocniejsza, niż §7.7 pkt 4 obiecywał:** zakaz progu absolutnego na prognozie przestaje być lintem i staje się kształtem typu — `Outlook` nie ma pola pieniężnego, a `sim/firms` nie ma czym go dodać |
| `BE-2`★ | **`MacroCell.labor` i `skill_sum` są wektorami, nie tablicami `[u32; N_ROLES]`**, a `MacroFirm` dostaje pole `cost: SparseVec<GoodId, Money>`, którego kontrakt M10 §6 nie wymienia | Liczba ról pochodzi z `data/jobs/roles.ron` (`K-43`) i jest **daną, nie stałą kompilacji** — katalog urósł w M7a z 4 do 46 i urośnie dalej, a tablica o stałym rozmiarze byłaby drugą deklaracją tej samej liczby. Koszt jednostkowy: faza 5 woła `kernel::next_price`, a ta bierze `unit_cost` jako **pierwsze** wejście; bez tego pola cenę trzeba by wyprowadzać z niej samej przez marżę docelową, czyli zgadywać koszt z ceny, którą ten koszt ma dopiero ustalić |
| `BE-3`★ | **`MacroState.ledger` nie jest `economy::Ledger`, tylko trzema kontami (`MacroLedger`).** Kontrakt M10a §5.6 mówi „ten sam typ co mezo", a typ mezo (M5c §5.8) jest księgą **jednego zakładu** — ma `site`, `firm` i miesięczne domknięcia | Makro potrzebuje sumy całego miasta, nie księgi zakładu. Wspólne zostaje to, co istotne: podział idzie przez `kernel::ledger_post`, a jedynym wejściem mutującym jest `transfer`, więc „ktoś dopisał pieniądz" jest w tym crate'cie **niewyrażalne**. Długu w tej księdze nie ma i nie będzie: dług nie jest pieniądzem, tylko zobowiązaniem, i siedzi w `MacroCell.debt` oraz `MacroFirm.debt` |
| `BE-4`★ | **`Scenario::KeepCourse` niesie `FirmId`.** §5.10 opisywał go jako pusty wariant | Wariant jest **punktem odniesienia**, a punkt odniesienia musi wiedzieć, co mierzy. Pusty `KeepCourse` zestawiał wynik firmy z zerem i wygrywał zawsze — złapał to pierwszy test rankingu (`ranking_nie_rozstrzyga_w_granicach_marginesu` zwracało `Some(1)` tam, gdzie warianty były identyczne) |
| `BE-5`★ | **`CommuteMatrix` jest płaska i to jest zapisany sufit, nie przeoczenie.** Każda para dzielnic kosztuje `FLAT_COMMUTE_MIN = 22` minuty | Kontrakt mówi „M10 trzyma, M4 wypełnia" (M10 §6) i tak zostaje. Konsekwencja **wiążąca dla faz 2 i 4**: firma zatrudnia i sprzedaje wyłącznie w swojej dzielnicy, bo przy płaskiej macierzy ograniczenie do dzielnicy jest jedyną rzeczą, która w ogóle odróżnia dzielnicę od dzielnicy. Kiedy M4 wypełni macierz, pula rozszerzy się o dzielnice w zasięgu dojazdu — i to jest cała zmiana, jakiej to będzie wymagało |
| `BE-6`★ | **§7.7 pkt 1 i 2 mierzą się inaczej, niż zapowiadały.** Plan żąda 200 zestawów po 3–5 wariantów porównanych z **faktycznym przebiegiem mezo** i ≥ 95 % zgodnych uporządkowań. Przebieg mezo pełnego miasta kosztuje **2,7 s na dobę**, więc 200 zestawów × 5 wariantów × 90 dób to ~65 godzin na jedno uruchomienie testu. Testuje się to, co da się zmierzyć: kierunek znany **z konstrukcji** (zamknięcie trwale nierentownego zakładu musi wygrać ponad margines), brak rozstrzygnięcia przy wariantach identycznych (100 %, `§7.7 pkt 3`) oraz **zależność marginesu od tego, od czego zależy wariancja** — rośnie z horyzontem, maleje z liczbą klientów | To jest przypadek (3) z `K-18`: kryterium niemierzalne. Margines stały przechodziłby każdy test porównawczy i nie mówiłby nic — dlatego test sprawdza jego **zmienność**, a nie wartość. Pełne uporządkowanie wobec mezo zostaje dla M10, który ma `dry_run()` i budżet stukrotnie szybszego przebiegu „na sucho"; adres: **M10a** |
| `BE-7` | **Bramki balansatora rosną o dwie: G10 (populacja firm w ±40 % wartości startowej) i G11 (mediana bezrobocia z ostatnich 30 dób w ⟨3 %, 12 %⟩, gdy wakatów jest nie więcej niż ludzi w sile roboczej). Obie liczą się z nowych pól `DayMetrics`, a `run::single` buduje świat przez `full::setup`, czyli z rejestrem firm, rynkiem pracy, upadłością i makrem.** G11 jest `nightly`, bo krótszy przebieg nie ma ogona, w którym miasto jest już zaludnione | Kryterium ukończenia WP15 brzmi „liczba firm nie eksploduje ani nie wymiera (pasmo z balansatora)" — a pasma w balansatorze nie było. Bez G10 to kryterium nie miało jak zapalić się na czerwono i rzeczywiście się nie zapaliło: pierwszy przebieg WP17 dał **215 → 271 firm w 150 dób**, czyli +62 % rocznie. Wynik po przestawieniu balansatora na pełne miasto: **G1–G11 zielone**, G4 doradcza (brak przebiegu szokowego). Determinizm (G8) też — dołożenie czterech systemów wyłącznych i modelu makro nie ruszyło ani kalibracji cen, ani powtarzalności |
| `BE-8`★ | **Powstawanie firm gaśnie **nasyceniem niszy**, a nie limitem dziennym.** `FoundingParams::min_people_per_shop` (350) mówi, ilu mieszkańców musi przypadać na sklep tego rodzaju w dzielnicy, żeby mieszkaniec zobaczył tam interes | Limit dzienny z §5.14 chroni przed **pikiem** — setką firm po jednym szoku (`R5`) — i nie chroni przed **dryfem**: jedna firma na dobę to 360 rocznie, a przy 215 firmach startowych miasto podwaja się w pół roku bez żadnego szoku. Zmierzone: bez progu 215 → 271 w 150 dób, z progiem 215 → 221. Nisza jest **wyczerpywalna** — każdy nowy sklep obniża liczbę ludzi na sklep — więc powstawanie zatrzymuje się samo, kiedy dzielnica jest obsłużona. To jest sprzężenie zwrotne, a nie sufit |
| `BE-9` | **Wypłata z masy upadłościowej dla wierzyciela bez gospodarstwa trafia do `Population::escheat`, a nie znika.** Poprawka w `corpfin::system::wyplac_sektorowi` (kod M7d) | `InsolvencySystem` po raz pierwszy stanął w przebiegu dopiero w M7f — do tego czasu mechanizm istniał, ale nikt go nie wołał. `continue` w pętli wypłat znaczyłby pieniądz zgubiony **między** księgami a światem: kwota zeszła już z ksiąg kanałem `household_sector_out`, a do komponentu nie doszła. Adresem takiego pieniądza jest pozycja, która istnieje dokładnie po to |
| `BE-10` | **Scenariusz nazywa się `m7miasto` i liczy w dobach, nie w latach.** §1 fazy zapowiadał `--scenario m7_miasto --years 5` | Podkomenda `clap` nie ma podkreślenia w nazwie, a doba jest jednostką, w której mierzy się wszystko inne w tym repo (`--days` w `m5shop`, w balansatorze, w bramkach). Pięć lat zapisuje się `--days 1800` i kosztuje ~80 minut w profilu `release` przy pełnej populacji |
