...
|---|---|---|---|---|
| ✅ **WP3** | Rdzeń `engine/ui` | WP1 | Dirty-flagging modelu przez `DataVersion` (`Cached`, `DataSource`), motyw w danych (`Theme`, `data/ui/theme.ron`), skala interfejsu i formatowanie liczb, pieniądza i dat per język, i18n PL/EN z pluralizacją (cztery formy PL), **`Span`/`Rich` z polem `link`** i **`TabStrip`**. Drzewo retained, własny układ, atlas fontów i listy rysowania **skreślone** — daje je `egui` (`DE-1`) | Panel testowy: brak zmian danych → 0 alokacji i 0 ms przebudowy; pseudo-lokalizacja ×1,4 nie rozwala układu; **`Rich` składa się w `String` bajt w bajt zgodny ze złotym wydrukiem sprzed zmiany — test porównujący starą i nową ścieżkę dla trzech istniejących kart** |
| ✅ **WP6** | `Table<T>`, wykresy, mapy cieplne mini | WP3 | Wirtualizowana tabela na 100k wierszy z sortowaniem/filtrem poza klatką, `Series` z piramidą mip (dzień/dekada/miesiąc/kwartał, kalendarz 12 × 30 wg K-1), miniatura mapy cieplnej. Graf i Gantt idą do `M9e` razem ze swoimi panelami (`DE-5`) | Budżety z §7 spełnione w criterion (`engine/ui/benches/ui_bench.rs`) |
...
## Zmiany wpisane w M9b

Zgodnie z `K-18`. Gwiazdka = zmiana zakresu albo kryterium. To są rzeczy, które wyszły
przy pierwszym czytaniu §5.8 obok kodu, który już stoi — czyli **przed** napisaniem
pierwszej linii, a nie po.

| # | Zmiana | Dlaczego |
|---|---|---|
| DE-1 ★ | **Drzewo retained z §5.8 nie powstaje. `trait Widget { measure / arrange / paint / event }`, `DrawList`, własny układ, własny atlas fontów i własna obsługa DPI są skreślone** — te rzeczy daje `egui` i daje je od M3d | §5.8 i `Z-1` mówiły dwie różne rzeczy o tym samym. `Z-1` (po M3d) rozstrzygnął: „rdzeniem jest `egui`, M9 dokłada panele, **nie integrację**", a ryzyko §8 dokumentu fazy uznano za rozbrojone **właśnie dlatego**. Postawienie drugiego stosu widgetów na `egui` byłoby dokładnie tym własnym toolkitem, którego tamta decyzja miała uniknąć — z tą różnicą, że dwa razy: raz nasz, raz cudzy pod spodem. Co z §5.8 zostaje bez zmian: `Span`/`Rich` z linkiem, `TabStrip`, `Table<T>`, `Series` z piramidą mip, `HeatmapThumb`, i18n, motyw w danych |
| DE-2 ★ | **Dirty-flagging przez `DataVersion` dotyczy modelu, nie drzewa widgetów** — i kryterium WP3 („brak zmian danych → 0 alokacji i 0 ms przebudowy") mierzy się na przebudowie **modelu**. `engine/ui` dostaje `DataSourceId`, `DataVersion` i `Cached<T>`; `Cached::get` przebudowuje wyłącznie wtedy, gdy wersja któregoś ze źródeł drgnęła | Tam leży koszt i jest zmierzony: `ShopCard::build` bierze zamek rynku i składa kilkaset napisów (`Market::shop_panel` 1,73 µs plus budowa karty), a `CitizenPanel::model` czyta świat. Rysowanie kilkuset widgetów `egui` jest przy tym szumem. Kryterium na drzewie widgetów mierzyłoby `egui`, a nie nas — czyli nie mierzyłoby niczego, na co mamy wpływ |
| DE-3 ★ | **Migawka dla paneli (`DB-1`) to `Cached<T>`, a nie druga kopia świata.** Podwójne buforowanie nie kupuje tu nic, bo symulacja i render chodzą w **jednym** wątku pętli klatki: model zbudowany po `Session::step` jest z definicji spójny na granicy ticku | `DB-1` zobowiązał WP3 do zbudowania migawki od zera i to zostaje — zmienia się tylko jej kształt. Sufit jest nazwany w kodzie: gdy symulacja wyjdzie na własny wątek (M12, tryb 50×), `Cached` staje się buforem tylnym i dochodzi zamiana wskaźników. Dzisiaj druga kopia byłaby kosztem bez konsumenta |
| DE-4 | **`LayoutNode` / `Layout` / dokowanie przenoszą się do `M9e`** razem z pierwszym panelem, który ma się gdzieś zadokować. `ViewCommand::SetLayout` zostaje w projekcie komend | Reguła kolejności z §4 dokumentu fazy: widget powstaje pod ekran, który go żąda. Ekrany powłoki są pełnoekranowe i nie dokują się do niczego, a karta inspekcji z `M9c` jest jednym oknem. Układ doków bez paneli to plik konfiguracyjny bez czytelnika |
| DE-5 | **`GraphView`, `GanttView` i `RuleEditorView` nie wchodzą do WP6**, a budżety §7 dokumentu fazy dla grafu i Gantta zamykają się razem ze swoimi panelami w `M9e`; `RuleEditorView` z edytorem w `M9d`. WP6 to `Table<T>`, `Series` i `HeatmapThumb` — i tyle mówi jego nazwa | Ten sam powód co `DE-4`. Graf łańcucha dostaw ma pierwszego konsumenta w panelu Łańcuch dostaw (`M9e` §5.9), Gantt w panelu Zakład, edytor w `M9d` §5.6. Zmierzenie ich teraz znaczyłoby zmierzenie widgetu, który rysuje dane zbudowane na potrzeby benchmarku |
| DE-6 | **Filtr `Table<T>` jest predykatem, nie `ConditionExpr`.** „Jedna gramatyka dla reguł, filtrów, celów i warunków zatrzymania" zostaje obietnicą **formy zapisu**, a nie typem pola w tabeli: `Table` bierze `Filter<T>` (domknięcie po wierszu), a `M9e` podłącza pod nie ewaluator `sim/policy` | `engine/ui` nie zależy od `sim/policy` i nie ma powodu zaczynać — tabela potrzebuje odpowiedzi „ten wiersz zostaje", a nie drzewa składniowego. Pole `ConditionExpr` w `Table` wymusiłoby, żeby **każda** tabela w grze była opisana AST-em, także lista dziesięciu slotów zapisu. Gramatyka nadal jest jedna: tam, gdzie filtr pisze gracz, pisze go w języku reguł, a ewaluator zwraca predykat |
| DE-7 | **Ekrany powłoki rysują się w `game/`, nie w `engine/ui`** (`game::screens`), bo `game` zależy od `ui`, nigdy odwrotnie, a modelem ekranów jest `game::shell` z M9a. `engine/ui` dostarcza im widgetów (`TabStrip`, `Theme`, segmenty, przycisk podstawowy) i na tym kończy się jego rola | §5.14 mówi „logika należy do WP13, WP14 tylko rysuje" i nie rozstrzyga adresu. Postawienie rysowania w `engine/ui` wymagałoby drugiego kompletu struktur widoku (`NewGameView` obok `NewGameParams`), które rozjechałyby się przy pierwszym nowym polu kreatora. `game/` dostaje przy tym zależność od `egui` — czystego procesora bez GPU, więc bramka R-5 nie drgnęła |
| DE-8 | **Polski dostaje czwartą formę liczebnika** (`other` z CLDR — ułamki: „1,5 sklepu"), więc wpisy `Plural` w `data/locale/{pl,en}.ron` mają cztery formy po stronie PL. `Catalog::plural` bez zmian, dochodzi `plural_frac` dla wartości ułamkowych | §5.8 mówi o czterech formach, kod M3 ma trzy, a kryterium §7 dokumentu fazy wymienia „1,5 sklepu" wprost. Kluczy liczebnikowych jest siedem, więc koszt jest żaden — a dopisanie formy później znaczyłoby przejrzenie wszystkich wpisów w chwili, gdy będzie ich kilkaset |
| DE-9 | **`Subject` wchodzi do `engine/core` teraz** (`K-62`, `Z-9`), z wariantami, których typy `core` już zna: `Citizen`, `Household`, `Firm`, `Site`, `Building`, `Parcel`, `Vehicle`, `Contract`, `District`, `Event`, `Government`. `Batch`, `Offer`, `Tender`, `Case` i `Permit` dokłada **`M9c`** razem z decyzją, gdzie mieszkają ich identyfikatory | `Span.link` z §5.8 jest `Option<Subject>`, więc typ musi istnieć w tej podfazie. Pozostałe pięć wariantów wymaga przeniesienia `TenderId`/`CaseId`/`PermitId` z `sim/city` do `core` albo uchwytu areny w `Subject` — to jest decyzja karty inspekcji, a nie rdzenia UI, i podejmuje ją faza, która te karty rysuje |
| DE-10 | **Sortowanie i filtrowanie `Table<T>` idzie na `std::thread`, nie na `engine/jobs`** — ta sama ścieżka i ten sam sufit co `WorldGenJob` (`DA-5`) | `JobPool` wystawia wyłącznie funkcje blokujące; uchwytu do zadania w tle nie ma. Wywołanie `scope` z wątku pętli klatki czekałoby na wynik, czyli robiłoby dokładnie to, czego budżet „poza klatką" zabrania |

Dwie korekty dopisane w trakcie pracy, obie o zmniejszeniu zakresu:

| # | Zmiana | Dlaczego |
|---|---|---|
| DE-11 | **Ekran ustawień ma dwie zakładki, nie cztery**: „Gra" i „Sterowanie". Grafika i dźwięk wchodzą razem ze swoją treścią w M11 | `ui-design.md` §6.5 wymienia cztery, ale dwie z nich nie mają dziś **ani jednego** ustawienia: rozdzielczość, tryb okna i jakość cieni należą do M11, dźwięku nie ma. Ten sam dokument mówi w §4, że zakładka pusta jest ukrywana, a nie wyszarzana — bo obiecuje treść, której nie ma |
| DE-12 | **Slot zapisu niesie majątek zero**, dopóki nie ma postaci gracza (WP4, `M9c`). Nazwą slotu jest nazwa pierwszej dzielnicy — własnej nazwy miasto nie ma | Wiersz listy slotów pokazuje „miasto · data · majątek · czas". Trzy z tych czterech liczb są prawdziwe; majątku nie ma skąd wziąć, bo `PlayerCharacter` powstaje dopiero w `M9c`, a wpisanie tam sumy kont gospodarstwa byłoby liczbą z sufitu. Zero z komentarzem jest uczciwsze niż liczba, która wygląda na majątek gracza |


## Zmiany wpisane po M9b

Zgodnie z `K-18`. Wyszły z gry uruchomionej po M9e — czyli z jedynego miejsca, w którym
widać ekrany powłoki złożone razem, a nie każdy osobno w teście.

| # | Zmiana | Dlaczego |
|---|---|---|
| DE-13 ★ | **Esc należy wyłącznie do `egui` — także nad grającym światem.** `App::klawisz` w `tools/magnat` wychodzi natychmiast, gdy `w_powloce()`, i nie ma już ramienia `Escape`; menu pauzy otwiera `buduj_ui` po tej samej stronie, po której je rysuje. Wyjście z menu głównego jest zwykłą pozycją listy plus Esc, tą samą drogą co mysz | Klawisz miał dwóch właścicieli i obaj widzieli **to samo** naciśnięcie w jednej klatce — `egui_winit` wkłada zdarzenie do kolejki niezależnie od tego, czy je pochłonęło. Skutek był widoczny u gracza: w kreatorze, w ustawieniach i na liście slotów Esc zamykał grę, a w grze otwierał pauzę, którą narysowane w tej samej klatce menu pauzy natychmiast zamykało. Sam warunek `w_powloce()` tego drugiego nie leczy: w klatce naciśnięcia gra jeszcze **nie jest** w powłoce, więc pętla okna zdąża otworzyć pauzę, zanim ekran ją zobaczy |
| DE-14 ★ | **`GameState::Shell` traci ładunek `ShellScreen`.** Który ekran powłoki jest na wierzchu, wie `Shell::screen` i tylko on | Klient aktualizował obie kopie w dwóch z pięciu przejść między ekranami, a warunek wyjścia z gry czytał tę nieaktualną. To nie była pomyłka w jednym miejscu, tylko drugie źródło prawdy, które musiało się rozjechać |
| DE-15 ★ | **Cofnięcie jest jedną tabelą, nie pięcioma odpowiedziami.** Ekran zwraca `ShellAction::Back` — samą intencję — a `Shell::cofnij` mówi dokąd. Wspólny nagłówek (`screens::controls::header`) rysuje klikalny przycisk „Wstecz" i ścieżkę od korzenia; ekran o ścieżce jednoelementowej przycisku nie dostaje | Cel cofnięcia był zapisany osobno w każdym ekranie, na ekranie wyboru postaci nie było go wcale, a w ustawieniach przycisk i klawisz prowadziły przez dwie różne gałęzie kodu do tego samego miejsca. Widoczne wyjście jest przy okazji kryterium `ui-design.md` §1: gracz, który nie wie o Esc, ma je widzieć |
| DE-16 | **Znaków spoza atlasu fontów nie ma w tekstach interfejsu.** Test `atlas_fontow_zna_wszystkie_znaki_z_lokalizacji` przechodzi po wszystkich formach wszystkich kluczy obu języków; strzałki `↑↓←→` wypadły z podpowiedzi klawiszy, z nagłówka karty podróży i ze znacznika wybranej opcji | Sprawdzenie atlasu było od M9b, ale mierzyło listę wpisaną do testu, czyli polskie diakrytyki. Znaku, którego nikt nie podejrzewał, lista z góry nie mogła złapać — i nie złapała: podpowiedź „↑↓ wybór · ←→ zmiana" wychodziła u gracza jako ciąg prostokątów. Strzałki wrócą, kiedy M11 wgra własny krój i test przestanie je odrzucać |
| DE-17 | **Lista slotów zatwierdza slot kliknięty, a nie ten pod kursorem klawiatury.** Ekran ma własną pętlę rysującą (wiersz slotu jest szerszy i dwukolorowy, więc nie przechodzi przez `menu_list`) i czytał indeks policzony **przed** nią | Usterka jest z M9a i dotyczy **zapisu**: klik w slot 5 w trybie „Zapisz" nadpisywał slot spod kursora, czyli zwykle zerowy. Klawiatura działała, mysz nie, a testy ekranów mierzyły do tej pory tylko klawiaturę. Doszedł test klikający po liście z kursorem wracającym przed każdym klikiem na zero — bez tego zepsuta lista zwraca numery, które się zmieniają, tylko z opóźnieniem o jeden |
